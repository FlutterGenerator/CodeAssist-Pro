...
import dev.mutwakil.codeassist.compose.preview.databinding.ActivityComposePreviewBinding
...
import com.tyron.builder.compiler.ComposeStaticCallback
import com.tyron.builder.project.IProjectManager
import dev.mutwakil.codeassist.compose.preview.R
//import com.itsaky.androidide.projects.builder.BuildService
import com.tyron.resources.R as ResourcesR
...
//        if (buildService.isBuildInProgress) {
          if(IProjectManager.getInstance().currentProject.isCompiling || IProjectManager.getInstance().currentProject.isIndexing){
              Log.w("","Build already in progress")
            return
        }
...
//        LOG.info("Running build task: {}", task)
        ComposeStaticCallback.callback = {
            status ->
            runOnUiThread {
                if (status== ComposeStaticCallback.Status.FAILED) {
