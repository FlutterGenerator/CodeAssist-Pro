...
public class CMRelaxNGElementDeclaration implements CMElementDeclaration {
  ...
  public Collection<CMAttributeDeclaration> getAttributes() {
    if (attributes == null) {
      attributes =
          new CMRelaxNGAttributeDeclarationCollector(this, pattern.getContent()).getAttributes();
    }
    return attributes;
  }
  ...
  public Collection<CMElementDeclaration> getElements() {
    if (elements == null) {
      elements =
          new CMRelaxNGElementDeclarationCollector(cmDocument, pattern.getContent()).getElements();
    }
    return elements;
  }
  ...
  public Collection<CMElementDeclaration> getPossibleElements(
    ...
    PatternMatcher matcher =
        new PatternMatcher(pattern, new ValidatorPatternBuilder(new SchemaPatternBuilder()));
  ...
  private Set<String> getRequiredElementNames() {
    if (requiredElementNames == null) {
      ...
      MyRequiredElementsFunction elementsFunction =
          new MyRequiredElementsFunction(requiredElementNames);
  ...
  public Collection<CMElementDeclaration> getPossibleRequiredElements() {
    ...
    if (possibleRequiredElementNames == null) {
      ...
      PossibleRequiredElementsFunction elementsFunction =
          new PossibleRequiredElementsFunction(possibleRequiredElementNames);
      getPattern().getContent().apply(elementsFunction);
    }
    ...
    List<CMElementDeclaration> sortedElements =
        possibelRequiredElements.stream().collect(Collectors.toList());
    Collections.sort(
        sortedElements,
        (e1, e2) -> {
          return ((CMElementDeclaration) e1)
              .getLocalName()
              .compareTo(((CMElementDeclaration) e2).getLocalName());
        });
