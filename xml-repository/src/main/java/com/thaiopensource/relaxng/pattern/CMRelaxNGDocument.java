 ...
 *
 * <p>NOTE : this class is hosted in 'com.thaiopensource.relaxng.pattern' because {@link Pattern}
 * implementation like {@link ElementPattern} are not public. Once
 * https://github.com/relaxng/jing-trang/issues/271 will be fixed we could move this class in
 * 'org.eclipse.lemminx.extensions.relaxng.contentmodel' package.
 *
 * @author Angelo ZERR
 */
public class CMRelaxNGDocument implements CMDocument {
  ...
  public CMRelaxNGDocument(
      String relaxNGURI, Pattern start, URIResolverExtensionManager resolverExtensionManager) {
    this.start = start;
  ...
  public CMElementDeclaration findCMElement(DOMElement element, String namespace) {
    ...
    for (int i = 0; i < paths.size(); i++) {
      ...
      if (i == 0) {
        ...
        declaration =
            declaration != null ? declaration.findCMElement(elt.getLocalName(), namespace) : null;
      }
      if (declaration == null) {
        break;
      }
  ...
  public LocationLink findTypeLocation(DOMNode originNode) {
    ...
    CMRelaxNGElementDeclaration elementDeclaration =
        (CMRelaxNGElementDeclaration) findCMElement(originElement, originElement.getNamespaceURI());
    ...
    if (originAttribute != null) {
      attributeDeclaration =
          (CMRelaxNGAttributeDeclaration) elementDeclaration.findCMAttribute(originAttribute);
    ...
    Locator locator =
        attributeDeclaration != null
            ? attributeDeclaration.getPattern().getLocator()
    ...
    DOMRange range =
        DOMUtils.isRelaxNGUriCompactSyntax(systemId)
            ? null
    ...
    Range targetRange =
        new Range(
            createValidPosition(locator.getLineNumber() - 1, locator.getColumnNumber() - 1),
  ...
  private synchronized DOMDocument getSynchDocument(String systemId) {
    ...
    document = DOMUtils.loadDocument(systemId, resolverExtensionManager);
  ...
  DOMNode findNodeAt(Locator locator) {
    ...
    try {
      int offset =
          targetSchema.offsetAt(
              new Position(locator.getLineNumber() - 1, locator.getColumnNumber() - 1));
