 ...
 *
 * <p><strong>Performance:</strong> The query operations perform in <i>O(log n)</i> where
 * <var>n</var> is the number of lines in the document. The modification operations roughly perform
 * in <i>O(l * log n)</i> where <var>n</var> is the number of lines in the document and <var>l</var>
 * is the sum of the number of removed, added or modified lines.
 *
 * @since 3.2
 */
public class TreeLineTracker implements ILineTracker {
   ...
   * subtree covered by the node. All nodes under the left subtree represent lines before, all nodes
   * under the right subtree lines after the current node.
   */
  private static final class Node {
    ...
    /**
     * The line offset in this node's line tree, or equivalently, the number of characters in the
     * left subtree.
     */
    int offset;
  ...
  /**
   * Returns the offset for the given line number. Note that the last line is always incomplete,
   * i.e. has the {@link #NO_DELIM} delimiter.
   *
   * @param line a line number
   ...
   *
   * @param parent the new parent of <code>child</code>, <code>null</code> to replace the root node
   * @param child the new child of <code>parent</code>, may be <code>null</code>
   * @param isLeftChild <code>true</code> if <code>child</code> shall become <code>parent</code>'s
   *     left child, <code>false</code> if it shall become <code>parent</code>'s right child
   */
  ...
  /**
   * A left rotation around <code>parent</code>, whose structural position is replaced by <code>node
   * </code>.
   *
  ...
  /**
   * A right rotation around <code>parent</code>, whose structural position is replaced by <code>
   * node</code>.
   *
  ...
  /**
   * A double left rotation, first rotating right around <code>node</code>, then left around <code>
   * parent</code>.
   *
  ...
  /**
   * A double right rotation, first rotating left around <code>node</code>, then right around <code>
   * parent</code>.
   *
  ...
  /**
   * Updates the balance information in the parent chain of node until it reaches the root or finds
   * a node whose balance violates the AVL constraint, which is the re-balanced.
   *
   ...
   * @param wasLeftChild <code>true</code> if the deletion happened on <code>node</code>'s left
   *     subtree, <code>false</code> if it occurred on <code>node</code>'s right subtree
   */
