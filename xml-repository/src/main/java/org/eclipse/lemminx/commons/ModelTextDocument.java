...
public class ModelTextDocument<T> extends TextDocument {
  ...
  public ModelTextDocument(
      TextDocumentItem document, BiFunction<TextDocument, CancelChecker, T> parse) {
    super(document);
    this.parse = parse;
  }

  public ModelTextDocument(
      String text, String uri, BiFunction<TextDocument, CancelChecker, T> parse) {
    super(text, uri);
    this.parse = parse;
  }

  /**
   * Returns the existing parsed model synchronized with last version of the text document and null
   * otherwise.
   *
   * @return the existing parsed model synchronized with last version of the text document and null
   *     otherwise.
   */
  ...
  /**
   * Return the existing parsed model synchronized with last version of the text document or parse
   * the model.
   *
   * @return the existing parsed model synchronized with last version of the text document or parse
   *     the model.
  ...
  private synchronized T getSynchronizedModel() {
    ...
    try {
      ...
      LOGGER.fine(
          "Stop parsing parsing of model with version '"
              + version
              + "' in "
      ...
      LOGGER.fine(
          "End parse of model with version '"
              + version
              + "' in "
              + (System.currentTimeMillis() - start)
              + "ms");
