...
/**
 * Watches the parent process PID and invokes exit if it is no longer available. This implementation
 * waits for periods of inactivity to start querying the PIDs.
 */
public final class ParentProcessWatcher
    implements Runnable, Function<MessageConsumer, MessageConsumer> {
  ...
  /**
   * Checks whether the parent process is still running. If not, then we assume it has crashed, and
   * we have to terminate the Java Language Server.
   *
  ...
  private boolean parentProcessStillRunning() {
    ...
    try {
      ...
      if (!finished) {
        process.destroy();
        finished =
            process.waitFor(POLL_DELAY_SECS, TimeUnit.SECONDS); // wait for the process to stop
      }
      if (isWindows && finished && process.exitValue() > 1) {
        // the tasklist command should return 0 (parent process exists) or 1 (parent process doesn't
        // exist)
      ...
      if (process != null) {
        ...
        // It is only closed when the Process object is garbage collected (in its finalize()
        // method).
