...
public class XMLWorkspaceService implements WorkspaceService, IXMLCommandService {
      ...
      if (handler == null) {
        throw new ResponseErrorException(
            new ResponseError(
                ...
                "No command handler for the command: " + params.getCommand(),
                null));
      }
          ...
          cancelChecker -> {
            try {
              return handler.executeCommand(
                  params, xmlLanguageServer.getSharedSettings(), cancelChecker);
            } catch (Exception e) {
  ...
  public void didChangeWatchedFiles(DidChangeWatchedFilesParams params) {
    XMLTextDocumentService xmlTextDocumentService =
        (XMLTextDocumentService) xmlLanguageServer.getTextDocumentService();
    ...
    synchronized (commands) {
      if (commands.containsKey(commandId)) {
        throw new IllegalArgumentException(
            "Command with id '" + commandId + "' is already registered");
      }
  ...
  public void endCommandsRegistration() {
    if (!commands.isEmpty()) {
      xmlLanguageServer
          ...
          .registerExecuteCommand(new ArrayList<>(commands.keySet()));
    }
  }
}
