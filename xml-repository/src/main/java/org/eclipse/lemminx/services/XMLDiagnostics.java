...
class XMLDiagnostics {
  ...
  public DiagnosticsResult doDiagnostics(
    ...
    doExtensionsDiagnostics(
        xmlDocument, diagnostics, validationSettings, validationArgs, cancelChecker);
    return diagnostics;
  }
   ...
   * @param monitor
   */
  private void doExtensionsDiagnostics(
      DOMDocument xmlDocument,
      List<Diagnostic> diagnostics,
      XMLValidationSettings validationSettings,
      Map<String, Object> validationArgs,
      CancelChecker monitor) {
    for (IDiagnosticsParticipant diagnosticsParticipant :
        extensionsRegistry.getDiagnosticsParticipants()) {
      ...
      try {
        ...
        LOGGER.log(
            Level.SEVERE,
            "Error while processing diagnostics for the participant '"
                + diagnosticsParticipant.getClass().getName()
                + "'.",
            e);
      }
    }
