  ...
  public void formatDocType(
    ...
    if (isDTD) {
      ...
      replaceLeftSpacesWithIndentation(
          ...
          docType.getParentNode().getStart(),
          docType.getStart(),
          true,
          edits);
  ...
  private void formatDTDNodeDecl(
    ...
    int currentNewLineCount =
        XMLFormatterDocument.getExistingNewLineCount(
            formatterDocument.getText(), nodeDeclStart, formatterDocument.getLineDelimiter());
    if (currentNewLineCount > preservedNewLines) {
      ...
      replaceLeftSpacesWithIndentationWithMultiNewLines(
          indentLevel, parentNodeStart, nodeDeclStart, preservedNewLines + 1, edits);
    } else {
      ...
      replaceLeftSpacesWithIndentationWithMultiNewLines(
          indentLevel, parentNodeStart, nodeDeclStart, newLineCount, edits);
    }
