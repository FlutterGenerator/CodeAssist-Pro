...
public class DOMElementFormatter {
  ...
  public DOMElementFormatter(
      XMLFormatterDocument formatterDocument, DOMAttributeFormatter attributeFormatter) {
  ...
  public void formatElement(
    ...
    FormatElementCategory formatElementCategory =
        getFormatElementCategory(element, parentConstraints);
    ...
    if (emptyElements == EmptyElements.ignore) {
      ...
      if (element.hasEndTag()
          && (element.getEndTagOpenOffset() > start
      ...
      case MixedContent:
        ...
        int parentStartCloseOffset =
            element.getParentElement() != null
                ? element.getParentElement().getStartTagCloseOffset() + 1
                : 0;
        if ((parentStartCloseOffset != startTagOpenOffset
            && StringUtils.isWhitespace(
                formatterDocument.getText(), parentStartCloseOffset, startTagOpenOffset))) {
          replaceLeftSpacesWithIndentationPreservedNewLines(
              parentStartCloseOffset, startTagOpenOffset, indentLevel, edits);
          parentConstraints.setAvailableLineWidth(getMaxLineWidth());
          width += indentLevel * getTabSize();
        }
        break;
      case IgnoreSpace:
        if (element.getParentNode().isOwnerDocument()
        ...
        replaceLeftSpacesWithIndentationPreservedNewLines(
            0, startTagOpenOffset, indentLevel, edits);
        width += indentLevel * getTabSize();
        parentConstraints.setAvailableLineWidth(getMaxLineWidth());
        break;
      case NormalizeSpace:
        ...
        case collapse:
          {
            ...
            if (!element.isSelfClosed()
                && (end == -1 || element.getEndTagOpenOffset() + 1 < end)
    ...
    if (element.hasAttributes()) {
      ...
      for (DOMAttr attr : attributes) {
        ...
        attributeFormatter.formatAttribute(
            attr, prevOffset, singleAttribute, true, isFirstAttr, parentConstraints, edits);
        isFirstAttr = false;
   ...
   */
  private int formatElementStartTagOrSelfClosed(
      DOMElement element, XMLFormattingConstraints parentConstraints, List<TextEdit> edits) {
    // <foo| >
    // <foo| />
    ...
    switch (formatElementCategory) {
      ...
      case MixedContent:
        ...
        if (lastChild != null
          ...
          replaceLeftSpacesWithIndentationPreservedNewLines(
              startTagCloseOffset, endTagOpenOffset, indentLevel, edits);
          width += indentLevel * getTabSize();
        }
        break;
      case IgnoreSpace:
  ...
  private EmptyElements getEmptyElements(
      DOMElement element, FormatElementCategory formatElementCategory) {
  ...
  private boolean shouldFormatClosingBracketNewLine(DOMElement element) {
    boolean isSingleAttribute =
        element.getAttributeNodes() != null ? element.getAttributeNodes().size() == 1 : true;
  ...
  private void replaceLeftSpacesWithIndentationPreservedNewLines(
      int spaceStart, int spaceEnd, int indentLevel, List<TextEdit> edits) {
    formatterDocument.replaceLeftSpacesWithIndentationPreservedNewLines(
        spaceStart, spaceEnd, indentLevel, edits);
  }

  private void replaceLeftSpacesWithIndentationWithOffsetSpaces(
      int spaceCount, int from, int to, List<TextEdit> edits) {
    formatterDocument.replaceLeftSpacesWithIndentationWithOffsetSpaces(
        spaceCount, from, to, true, edits);
  }
