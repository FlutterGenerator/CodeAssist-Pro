...
public class XMLFormatterDocument {
  ...
  public void formatAttributeValue(
      DOMAttr attr, XMLFormattingConstraints parentConstraints, List<TextEdit> edits) {
    if (formatterParticipants != null) {
      for (IFormatterParticipant formatterParticipant : formatterParticipants) {
        try {
          if (formatterParticipant.formatAttributeValue(
              attr, this, parentConstraints, getFormattingSettings(), edits)) {
            return;
          }
        } catch (Exception e) {
          LOGGER.log(
              ...
              "Error while processing format attributes for the participant '"
                  + formatterParticipant.getClass().getName()
                  + "'.",
              e);
        }
      }
  ...
  public void replaceLeftSpacesWithIndentationPreservedNewLines(
    ...
    int currentNewLineCount =
        XMLFormatterDocument.getExistingNewLineCount(
    ...
    if (currentNewLineCount > preservedNewLines) {
      replaceLeftSpacesWithIndentationWithMultiNewLines(
          indentLevel, spaceStart, spaceEnd, preservedNewLines + 1, edits);
    } else {
      int newLineCount = currentNewLineCount == 0 ? 1 : currentNewLineCount;
      replaceLeftSpacesWithIndentationWithMultiNewLines(
          indentLevel, spaceStart, spaceEnd, newLineCount, edits);
    }
  }
  ...
  public FormatElementCategory getFormatElementCategory(
    ...
    for (IFormatterParticipant participant : formatterParticipants) {
      FormatElementCategory fromParticipant =
          participant.getFormatElementCategory(
              element, parentConstraints, formattingContext, sharedSettings);
