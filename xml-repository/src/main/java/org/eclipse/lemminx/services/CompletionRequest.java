...
class CompletionRequest extends AbstractPositionRequest implements ICompletionRequest {
    ...
    if (generator == null) {
      generator =
          new XMLGenerator(
              getSharedSettings(),
              isAutoCloseTags(),
              getLineIndentInfo().getWhitespacesIndent(),
              getLineIndentInfo().getLineDelimiter(),
  ...
  public boolean isResolveDocumentationSupported() {
    XMLCompletionSettings completionSettings = sharedSettings.getCompletionSettings();
    return completionSettings.isCompletionResolveSupported(
        CompletionResolveSupportProperty.documentation);
  }
  ...
  public boolean isResolveAdditionalTextEditsSupported() {
    XMLCompletionSettings completionSettings = sharedSettings.getCompletionSettings();
    return completionSettings.isCompletionResolveSupported(
        CompletionResolveSupportProperty.additionalTextEdits);
  }
}
