...
class XMLSymbolsProvider {
  ...
  private static class SymbolsProviderParticipantResult {
    ...
    public SymbolsProviderParticipantResult(
        Collection<ISymbolsProviderParticipant> replaceParticipants,
        Collection<ISymbolsProviderParticipant> insertParticipants) {
      this.replaceParticipants =
          replaceParticipants != null ? replaceParticipants : Collections.emptyList();
      this.insertParticipants =
          insertParticipants != null ? insertParticipants : Collections.emptyList();
    }
  ...
  public SymbolInformationResult findSymbolInformations(
      DOMDocument xmlDocument, XMLSymbolSettings symbolSettings, CancelChecker cancelChecker) {
    AtomicLong limit =
        symbolSettings.getMaxItemsComputed() >= 0
    ...
    try {
      ...
      for (DOMNode node : xmlDocument.getRoots()) {
        try {
          ...
          LOGGER.log(
              Level.SEVERE,
              "XMLSymbolsProvider#findSymbolInformations was given a BadLocation by a 'node'"
                  + " variable",
              e);
        }
      }
  ...
  private void findSymbolInformations(
      DOMNode node,
      String container,
      List<SymbolInformation> symbols,
      boolean ignoreNode,
      XMLSymbolFilter filter,
      boolean hasFilterForAttr,
    ...
    if (!ignoreNode) {
      ...
      SymbolInformation symbol =
          new SymbolInformation(name, getSymbolKind(node), location, container);
      symbols.add(symbol);
    }
    ...
    node.getChildren()
        .forEach(
            child -> {
              try {
                findSymbolInformations(
                    child, containerName, symbols, false, filter, hasFilterForAttr, cancelChecker);
              } catch (BadLocationException e) {
                LOGGER.log(
                    Level.SEVERE,
                    "XMLSymbolsProvider was given a BadLocation by the provided 'node' variable",
                    e);
              }
            });
  }
  ...
  public DocumentSymbolsResult findDocumentSymbols(
      DOMDocument xmlDocument, XMLSymbolSettings symbolSettings, CancelChecker cancelChecker) {
    AtomicLong limit =
        symbolSettings.getMaxItemsComputed() >= 0
            ? new AtomicLong(symbolSettings.getMaxItemsComputed())
          ...
          .forEach(
              node -> {
                try {
                  ...
                  findDocumentSymbols(
                      node, symbols, nodesToIgnore, filter, hasFilterForAttr, cancelChecker);
                } catch (BadLocationException e) {
                  LOGGER.log(
                      Level.SEVERE,
                      "XMLSymbolsProvider#findDocumentSymbols was given a BadLocation by a 'node'"
                          + " variable",
                      e);
                }
              });
  ...
  private void findDocumentSymbols(
      DOMNode node,
      DocumentSymbolsResult symbols,
      List<DOMNode> nodesToIgnore,
      XMLSymbolFilter filter,
      boolean hasFilterForAttr,
      CancelChecker cancelChecker)
      throws BadLocationException {
    ...
    if (nodesToIgnore == null || !nodesToIgnore.contains(node)) {
      ...
      if (nodesToIgnore != null
          && node.isDTDAttListDecl()) { // attlistdecl with no elementdecl references
      ...
      childrenSymbols =
          hasChildNodes || node.isDTDElementDecl() || node.isDTDAttListDecl() || collectAttributes
              ? symbols.createList()
              : DocumentSymbolsResult.EMPTY_LIMITLESS_LIST;
      DocumentSymbol symbol =
          new DocumentSymbol(
              name, getSymbolKind(node), range, selectionRange, null, childrenSymbols);
      symbols.add(symbol);
      ...
      if (node.isElement()) {
        if (collectAttributes) {
          ...
          for (DOMAttr attr : node.getAttributeNodes()) {
            findDocumentSymbols(
                attr, childrenSymbols, attrToIgnore, filter, hasFilterForAttr, cancelChecker);
          }
        }
      } else {
        if (node.isDTDElementDecl() || (nodesToIgnore != null && node.isDTDAttListDecl())) {
          ...
          for (DOMNode attrDecl : attlistDecls) {
            findDocumentSymbols(
                attrDecl, childrenSymbols, null, filter, hasFilterForAttr, cancelChecker);
            if (attrDecl instanceof DTDAttlistDecl) {
              ...
              if (otherAttributeDecls != null) {
                for (DTDAttlistDecl internalDecl : otherAttributeDecls) {
                  findDocumentSymbols(
                      internalDecl, childrenSymbols, null, filter, hasFilterForAttr, cancelChecker);
                }
              }
            }
    ...
    node.getChildren()
        .forEach(
            child -> {
              try {
                findDocumentSymbols(
                    child, childrenOfChild, nodesToIgnore, filter, hasFilterForAttr, cancelChecker);
              } catch (BadLocationException e) {
                LOGGER.log(
                    Level.SEVERE,
                    "XMLSymbolsProvider was given a BadLocation by the provided 'node' variable",
  ...
  private List<DOMNode> getFilteredNodeAttributes(
      DOMNode node, XMLSymbolFilter filter, boolean hasFilterForAttr) {
    if (!hasFilterForAttr) {
      return null;
    }
  ...
  private static Range getSymbolRange(DOMNode node, boolean useAttlistElementName)
      throws BadLocationException {
    Position start;
    Position end;
    DOMDocument xmlDocument = node.getOwnerDocument();
  ...
  private static String nodeToNameOrNull(
      DOMNode node, XMLSymbolFilter filter, boolean hasFilterForAttr) {
    if (node.isElement()) {
  ...
  private boolean processSymbolsParticipants(
      DOMDocument xmlDocument,
      SymbolInformationResult symbolInformations,
      DocumentSymbolsResult documentSymbols,
      XMLSymbolFilter filter,
      CancelChecker cancelChecker) {
    ...
    Collection<ISymbolsProviderParticipant> replaceParticipants =
        resultParticipant.getReplaceParticipants();
    if (!replaceParticipants.isEmpty()) {
      boolean successfulReplace = false;
      if (replaceParticipants.size() > 1) {
        LOGGER.log(
            Level.WARNING, "There are '" + replaceParticipants.size() + "' replace participants");
      }
      for (ISymbolsProviderParticipant replaceParticipant : replaceParticipants) {
        try {
          if (symbolInformations != null) {
            replaceParticipant.findSymbolInformations(
                xmlDocument, symbolInformations, filter, cancelChecker);
          }
          if (documentSymbols != null) {
            replaceParticipant.findDocumentSymbols(
                xmlDocument, documentSymbols, filter, cancelChecker);
          }
          ...
          LOGGER.log(
              Level.SEVERE,
              "Error while processing symbols for '"
                  + replaceParticipant.getClass().getName()
                  + "'.",
              e);
        }
      }
    ...
    Collection<ISymbolsProviderParticipant> insertParticipants =
        resultParticipant.getInsertParticipants();
    if (!insertParticipants.isEmpty()) {
      for (ISymbolsProviderParticipant insertParticipant : insertParticipants) {
        try {
          if (symbolInformations != null) {
            insertParticipant.findSymbolInformations(
                xmlDocument, symbolInformations, filter, cancelChecker);
          }
          if (documentSymbols != null) {
            insertParticipant.findDocumentSymbols(
                xmlDocument, documentSymbols, filter, cancelChecker);
          }
        } catch (CancellationException e) {
          throw e;
  ...
  private SymbolsProviderParticipantResult getSymbolsProviderParticipant(DOMDocument xmlDocument) {
    Collection<ISymbolsProviderParticipant> all =
        extensionsRegistry.getSymbolsProviderParticipants();
