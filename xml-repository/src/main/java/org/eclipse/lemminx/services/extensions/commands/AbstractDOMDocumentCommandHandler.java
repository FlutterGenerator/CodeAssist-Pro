...
/**
 * Abstract command to work with a given {@link DOMDocument} filled in the first argument of the
 * command as {@link TextDocumentIdentifier}.
 *
 * @author Angelo ZERR
 */
public abstract class AbstractDOMDocumentCommandHandler implements IDelegateCommandHandler {
  ...
  public final Object executeCommand(
      ExecuteCommandParams params, SharedSettings sharedSettings, CancelChecker cancelChecker)
      throws Exception {
    TextDocumentIdentifier identifier =
        ArgumentsUtils.getArgAt(params, 0, TextDocumentIdentifier.class);
    String uri = identifier.getUri();
    DOMDocument document = documentProvider.getDocument(uri);
    if (document == null) {
      throw new UnsupportedOperationException(
          String.format(
              "Command '%s' cannot find the DOM document with the URI '%s'.",
              params.getCommand(), uri));
    }
   ...
   * Executes a command
   *
   * @param document the DOM document retrieve by the {@link TextDocumentIdentifier} argument.
   * @param params command execution parameters
   * @param sharedSettings the shared settings
   * @param cancelChecker check if cancel has been requested
   * @return the result of the command
   * @throws Exception the unhandled exception will be wrapped in <code>
   *     org.eclipse.lsp4j.jsonrpc.ResponseErrorException</code> and be wired back to the JSON-RPC
   *     protocol caller
   */
