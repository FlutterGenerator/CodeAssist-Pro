...
public class DOMAttributeFormatter {
  ...
  public void formatAttribute(
    ...
    if (attr.hasDelimiter()) {
      ...
      if (isMaxLineWidthSupported()) {
        ...
        if (isPreserveAttributeLineBreaks() && hasLineBreak(prevOffset, attr.getStart())) {
          ...
          availableLineWidth =
              getMaxLineWidth() - getTabSize() * (indentLevel + getSplitAttributesIndentSize());
        } else {
    ...
    if (!alreadyIndented) {
      ...
      if (isMaxLineWidthSupported()
          && parentConstraints.getAvailableLineWidth() < 0
  ...
  private void formatAttributeValue(
