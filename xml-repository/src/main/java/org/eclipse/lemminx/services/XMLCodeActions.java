...
public class XMLCodeActions {
  ...
  public List<CodeAction> doCodeActions(
    ...
    if (diagnostics != null) {
      for (Diagnostic diagnostic : diagnostics) {
        if (diagnostic != null) { // Never run this cycle if diagnostic is null
          for (ICodeActionParticipant codeActionParticipant :
              extensionsRegistry.getCodeActionsParticipants()) {
            cancelChecker.checkCanceled();
            try {
              CodeActionRequest request =
                  new CodeActionRequest(
                      diagnostic, range, document, extensionsRegistry, sharedSettings);
              ...
              LOGGER.log(
                  Level.SEVERE,
                  "Error while processing code actions for the participant '"
                      + codeActionParticipant.getClass().getName()
                      + "'.",
                  e);
            }
          }
    ...
    // The second pass is for CodeAction participants that have to create CodeActions independently
    // of diagnostics
    for (ICodeActionParticipant codeActionParticipant :
        extensionsRegistry.getCodeActionsParticipants()) {
      cancelChecker.checkCanceled();
      try {
        CodeActionRequest request =
            new CodeActionRequest(null, range, document, extensionsRegistry, sharedSettings);
  ...
  public CodeAction resolveCodeAction(
      ...
      CancelChecker cancelChecker)
      throws CancellationException {
    ResolveCodeActionRequest request =
        new ResolveCodeActionRequest(unresolved, document, extensionsRegistry, sharedSettings);
    ...
    for (ICodeActionParticipant codeActionParticipant :
      ...
      try {
        ...
        ICodeActionResolvesParticipant resolveCodeActionParticipant =
            codeActionParticipant.getResolveCodeActionParticipant(participantId);
        ...
        LOGGER.log(
            Level.SEVERE,
            "Error while processing resolve code action for the participant '"
                + codeActionParticipant.getClass().getName()
                + "'.",
            e);
      }
