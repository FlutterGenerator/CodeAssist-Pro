...
/**
 * Abstract class of {@link CompletionItem} to report an XML element completion item. This class
 * returns a {@link TextEdit} which contains the full element content (ex : <foo></foo>) or only the
 * start tag element (ex : <foo) if it exists an orphan end tag (ex : </foo>).
 *
 * <p>When it exists an orphan end tag, an aditional text edit is reported to update this end tag
 * with the start tag of the completion item.
 *
 * <p>// Samples with completion from Text node
 *
 * <p>// fo| ==> <foo attr=""></foo> // fo| </foo> ==> <foo attr="">
 *
 * <p>// Samples with completion from element
 *
 * <p>// <fo| ==> foo attr=""></foo> // <fo| </foo> ==> foo attr=""> // <fo|> ==> foo // <fo|/> ==>
 * foo // <fo| attr="" ==> foo
 *
...
public abstract class AbstractElementCompletionItem<S, G> extends CompletionItem {
  ...
  public AbstractElementCompletionItem(
      String tagName, S sourceElement, G generator, ICompletionRequest request) {
    ...
    boolean updateEndTagName =
        !generateFullElement && !request.getSharedSettings().isLinkedEditingEnabled();
   ...
   *
   * @return the tag name of the XML element completion item (ex : 'foo', 'x:foo').
   */
  ...
  /**
   * Returns the source element (ex : a DOM element, a grammar element declaration, etc) used to
   * generate the completion item.
   *
   * @return the source element (ex : a DOM element, a grammar element declaration, etc) used to
   *     generate the completion item.
   */
   ...
   *
   * @return the generator to generate the full XML element content and null otherwise.
   */
  ...
  private boolean updateTextEdit() {
    ...
    if (!hasOrphanEndTag && shouldGenerateFullElement) {
      ...
      boolean shouldStartTagClosed =
          isTextNode || element == null || !hasContentAfterTagName(element);
   ...
   *
   * @return true if client can support completion resolve of 'documentation' and false otherwise.
   */
   ...
   * @return true if client can support completion resolve of 'additionalTextEdits' and false
   *     otherwise.
   */
  private boolean updateAdditionalTextEdits(boolean updateEndTagName) {
    ...
    if (updateEndTagName) {
      updateEndTagName(request.getNode(), request.getOffset(), request, tagName, this);
    }
    return false;
  }
   ...
   *
   * @param generateEndTag true if the element end tag must be generated and false otherwise.
  ...
  public static void updateEndTagName(
      DOMNode node,
      int offset,
      ISharedSettingsRequest request,
      String tagName,
      CompletionItem completionItem) {
    List<TextEdit> additionalTextEdits = getAdditionalTextEdits(node, offset, tagName, request);
    if (additionalTextEdits != null) {
      completionItem.setAdditionalTextEdits(additionalTextEdits);
    }
  }

  private static List<TextEdit> getAdditionalTextEdits(
      DOMNode node, int offset, String tagName, ISharedSettingsRequest request) {
    ...
    if (isTextNode(node, offset)) {
      ...
      endElement =
          element.hasEndTag()
              ? element
              : element.getOrphanEndElement(endElementOffset, tagName, true);
    }
