 ...
 * </ul>
 *
 * Here a data sample for codeAction used by the resolve code action: <code>
 * {
...
public class DataEntryField {
   ...
   *
   * @param uri the URI of the document XML where completion, codeAction, etc must be resolved (when
   *     user clicks on this quickfix).
   * @param participantId the participant ID which must be used to process the resolve of the
   *     completion, codeAction, etc.
  ...
  /**
   * Returns the URI of the document XML where completion, codeAction, etc must be resolved (when
   * user clicks on this quickfix) and null otherwise.
   *
   * @param data the data entry field comings from the CompletionItem, CodeAction, etc.
   * @return the URI of the document XML where completion, codeAction, etc must be resolved (when
   *     user clicks on this quickfix) and null otherwise.
   */
  ...
  /**
   * Returns the participant ID which must be used to process the resolve of the completion,
   * codeAction, etc and null otherwise.
   *
   * @param data the data entry field comings from the CompletionItem, CodeAction, etc.
   * @return the participant ID which must be used to process the resolve of the completion,
   *     codeAction, etc and null otherwise.
   */
  ...
  /**
   * Returns the property value of the given <code>fieldName</code> if the given <code>data</code>
   * and null otherwise.
   *
   * @param data the data entry field comings from the CompletionItem, CodeAction, etc.
   * @param fieldName the entry field name (ex : "uri", "participantId").
   * @return the property value of the given <code>fieldName</code> if the given <code>data</code>
   *     and null otherwise.
   */
  ...
  public static JsonObject createCompletionData(ICompletionRequest request, String participantId) {
    ...
    JsonObject data = createData(document.getDocumentURI(), participantId);
    data.addProperty(OFFSET_FIELD, request.getOffset());
    return data;
  }
}
