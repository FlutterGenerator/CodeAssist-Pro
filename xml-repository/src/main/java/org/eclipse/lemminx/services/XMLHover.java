...
class XMLHover {
  ...
  public Hover doHover(
    ...
    if (node.isElement() && ((DOMElement) node).hasTagName()) {
      ...
      if (element.hasEndTag() && offset >= element.getEndTagOpenOffset()) {
        Range tagRange =
            getTagNameRange(TokenType.EndTag, element.getEndTagOpenOffset(), offset, xmlDocument);
        if (tagRange != null) {
   ...
   * @param open true if it's the start tag which is hovered and false if it's the end tag.
   * @param cancelChecker the cancel checker.
   * @return the LSP hover from the hovered element.
   */
  private Hover getTagHover(
      HoverRequest hoverRequest, Range tagRange, boolean open, CancelChecker cancelChecker) {
  ...
  private Range getTagNameRange(
      TokenType tokenType, int startOffset, int offset, DOMDocument document) {
    ...
    if (token == tokenType && offset <= scanner.getTokenEnd()) {
      try {
        ...
        LOGGER.log(
            Level.SEVERE,
            "While creating Range in XMLHover the Scanner's Offset was a BadLocation",
            e);
        return null;
      }
    }
   ...
   */
  private Hover getAttrNameHover(
      HoverRequest hoverRequest, Range attrRange, CancelChecker cancelChecker) {
   ...
   * @return the LSP hover from the hovered attribute.
   */
  private Hover getAttrValueHover(
      HoverRequest hoverRequest, Range attrRange, CancelChecker cancelChecker) {
    hoverRequest.setHoverRange(attrRange);
    List<Hover> hovers = new ArrayList<>();
    for (IHoverParticipant participant : extensionsRegistry.getHoverParticipants()) {
   ...
   */
  private Hover getTextHover(
      HoverRequest hoverRequest, Range textRange, CancelChecker cancelChecker) {
  ...
  private static Hover mergeHover(List<Hover> hovers, Range defaultRange) {
    ...
    List<MarkupContent> contents =
        hovers.stream() //
            .filter(
                hover ->
                    hover.getContents() != null
                        && hover.getContents().isRight()
                        && hover.getContents().getRight() != null) //
            .map(hover -> hover.getContents().getRight())
