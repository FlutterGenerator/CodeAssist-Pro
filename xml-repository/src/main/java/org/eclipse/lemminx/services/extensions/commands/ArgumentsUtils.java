...
public class ArgumentsUtils {
   ...
   * @return the object from the given index as a given class type and null otherwise.
   * @throws UnsupportedOperationException if the object from the given index cannot be retrieved
   *     from the params.
   */
  public static <T> T getArgAt(ExecuteCommandParams params, int index, Class<T> clazz) {
    ...
    if (obj == null) {
      throw new UnsupportedOperationException(
          String.format(
              "Command '%s' must be called with '%s' argument(s)!",
