...
class XMLLinkedEditing {
  ...
  public LinkedEditingRanges findLinkedEditingRanges(
      DOMDocument document, Position position, CancelChecker cancelChecker) {
    try {
      cancelChecker.checkCanceled();

      ILinkedEditingRangesRequest request =
          new LinkedEditingRangesRequest(document, position, extensionsRegistry);
      ...
      for (ILinkedEditingRangesParticipant participant :
          extensionsRegistry.getLinkedEditingRangesParticipants()) {
        try {
      ...
      if (node.isElement()) {
        ...
        if (!(element.isOrphanEndTag() || !element.hasEndTag())) {
          ...
          if (element.isInStartTag(offset) || element.isInEndTag(offset, true)) {
            ranges.addAll(
                Arrays.asList(
                    XMLPositionUtility.selectStartTagName(element),
