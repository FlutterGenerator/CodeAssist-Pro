...
public class DOMTextFormatter {
  ...
  public void formatText(
      ...
      while (i >= 0) {
        ...
        if (isLineSeparator(curr)) {
          ...
          lineSeparatorOffset = i;
            ...
            && (!Character.isWhitespace(curr) || isLineSeparator(curr))
        ...
        if (isMaxLineWidthSupported()) {
          ...
          if (availableLineWidth < 0 && spaceStart != -1) {
            ...
            replaceLeftSpacesWithIndentation(
                mixedContentIndentLevel, spaceStart, contentStart, true, edits);
            availableLineWidth =
                maxLineWidth - (contentEnd - contentStart) - mixedContentIndentLevel * getTabSize();
            ...
            availableLineWidth =
                maxLineWidth - (contentEnd - contentStart) - indentLevel * getTabSize();
          }
        }
    ...
    if (formatElementCategory != FormatElementCategory.IgnoreSpace
        && spaceEnd + 1 != text.length()) {
      ...
      if ((!containsNewLine || isJoinContentLines() || isMixedContent)
      ...
      } else if (isMaxLineWidthSupported()
          && availableLineWidth < 0
        ...
        int mixedContentIndentLevel =
            parentConstraints.getMixedContentIndentLevel() == 0
                ...
                : parentConstraints.getMixedContentIndentLevel();
        replaceLeftSpacesWithIndentationPreservedNewLines(
            textStart, textStart, mixedContentIndentLevel, edits);
        availableLineWidth =
            getMaxLineWidth() - (textEnd - textStart) - mixedContentIndentLevel * getTabSize();
      } else {
        ...
        replaceLeftSpacesWithIndentationPreservedNewLines(
            spaceStart, spaceEnd + 1, indentLevel, edits);
        if (isMaxLineWidthSupported()) {
          availableLineWidth =
              getMaxLineWidth() - (textEnd - textStart) - indentLevel * getTabSize();
        }
      }
