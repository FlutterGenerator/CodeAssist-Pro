...
public class AttributeCompletionItem extends CompletionItem {
   ...
   * @param canSupportSnippets true if snippets is supported to generate attribute value and false
   *     otherwise
   * @param fullRange the range to edit.
   * @param generateValue true if attribute value must be generated and false otherwise.
   ...
   */
  public AttributeCompletionItem(
      String attrName,
    ...
    if (generateValue) {
      ...
      String attributeValue =
          XMLGenerator.generateAttributeValue(
              defaultValue, enumerationValues, canSupportSnippets, 1, true, sharedSettings);
      attributeContent.append(attributeValue);
    }
    super.setTextEdit(Either.forLeft(new TextEdit(fullRange, attributeContent.toString())));
    super.setInsertTextFormat(
        canSupportSnippets ? InsertTextFormat.Snippet : InsertTextFormat.PlainText);
  }
}
