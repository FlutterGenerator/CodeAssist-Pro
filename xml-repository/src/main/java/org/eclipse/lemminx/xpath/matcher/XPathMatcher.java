...
/**
 * XPath matcher is used to match if a DOM Node match a XPath expression. It is initialized with an
 * XPath expression :
 *
 * <ul>
 *   <li>static expression like //element, //element/@attr, //element/text()
 *   <li>expression with wildcard like //element[@id='$0']
 * </ul>
 */
public class XPathMatcher extends ArrayList<IXPathNodeMatcher> {
  ...
  /**
   * Returns true if the given DOM Node match the XPath expression of this XPath matcher and false
   * otherwise.
   *
   * @param node the DOM Node to match.
   * @return true if the given DOM Node match the XPath expression of this XPath matcher and false
   *     otherwise.
   */
  ...
  /**
   * Returns true if the given DOM Node match the XPath expression of this XPath matcher and false
   * otherwise. This method use wilcard values if XPath expression contains wildcard (like
   * [@id='$0'].
   *
   * @param node the DOM Node to match.
  ...
  private IXPathNodeMatcher createNodeMatcher(String prefix, String localName) {
    if (TEXT_NODE_SELECTOR.equals(localName)) {
      ...
      // ex : @id
      return new XPathAttributeNameMatcher(
          prefix, localName.substring(1, localName.length()), this);
  ...
  protected XPathElementMatcher createAnyElementMatcher() {
    XPathElementMatcher matcher =
        new XPathElementMatcher(null, XPathElementMatcher.ANY_ELEMENT_NAME, this);
    super.add(matcher);
    return matcher;
  }
  ...
  protected XPathAttributeMatcher createAttributeMatcher(
      XPathElementMatcher elementmatcher, String attrName, String attrValue) {
