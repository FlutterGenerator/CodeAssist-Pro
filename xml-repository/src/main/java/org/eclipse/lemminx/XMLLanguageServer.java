 ...
 *
 * <p>Contributors: Angelo Zerr <angelo.zerr@gmail.com> - initial API and implementation Red Hat
 * Inc. - Dynamic Server capabilities
 */
...
public class XMLLanguageServer
    implements ProcessLanguageServer,
  ...
  public CompletableFuture<InitializeResult> initialize(InitializeParams params) {
    ...
    ExtendedClientCapabilities extendedClientCapabilities =
        InitializationOptionsExtendedClientCapabilities.getExtendedClientCapabilities(params);
    capabilityManager.setClientCapabilities(params.getCapabilities(), extendedClientCapabilities);

    xmlTextDocumentService.updateClientCapabilities(
        capabilityManager.getClientCapabilities().capabilities,
    ...
    ServerCapabilities nonDynamicServerCapabilities =
        ServerCapabilitiesInitializer.getNonDynamicServerCapabilities(
            capabilityManager.getClientCapabilities(),
            xmlTextDocumentService.isIncrementalSupport());
  ...
  private synchronized void updateSettings(Object initOptions, boolean initLogs) {
    ...
    XMLGeneralClientSettings xmlClientSettings =
  ...
  public CompletableFuture<AutoCloseTagResponse> closeTag(TextDocumentPositionParams params) {
    return xmlTextDocumentService.computeDOMAsync(
        ...
        (xmlDocument, cancelChecker) -> {
          return getXMLLanguageService()
              .doAutoClose(
                  xmlDocument,
                  params.getPosition(),
                  getSharedSettings().getCompletionSettings(),
                  cancelChecker);
        });
  }
  ...
  public CompletableFuture<Position> matchingTagPosition(TextDocumentPositionParams params) {
    return xmlTextDocumentService.computeDOMAsync(
        ...
        (xmlDocument, cancelChecker) -> {
          return getXMLLanguageService()
              .getMatchingTagPosition(xmlDocument, params.getPosition(), cancelChecker);
        });
  }
  ...
  public void sendNotification(String message, MessageType messageType, Command... commands) {
    ...
    if (sharedSettings.isActionableNotificationSupport()
        && sharedSettings.isOpenSettingsCommandSupport()) {
      ActionableNotification notification =
          new ActionableNotification()
              .withSeverity(messageType)
              .withMessage(message)
              .withCommands(Arrays.asList(commands));
  ...
  public void notifyProgress(String progressId, WorkDoneProgressNotification notification) {
    ProgressParams params =
        new ProgressParams(Either.forLeft(progressId), Either.forRight(notification));
