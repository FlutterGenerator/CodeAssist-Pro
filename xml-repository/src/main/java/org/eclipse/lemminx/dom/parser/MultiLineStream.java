...
public class MultiLineStream {
  ...
  private static final Predicate<Integer> WHITESPACE_PREDICATE =
      ch -> {
  ...
  /**
   * Will advance the stream position until 'closingBracket' or using a stack to consider possible
   * open/closed bracket pairs in between.
   *
   * <p>'closingBracket' should be the closing bracket eg: > | ]
   */
  ...
  /**
   * Advances until it matches int[] ch OR it hits '<' If this returns true, peek if next char is
   * '<' to check which case was hit
   */
