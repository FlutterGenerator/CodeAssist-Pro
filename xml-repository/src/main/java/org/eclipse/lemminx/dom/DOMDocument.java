 ...
 *
 * <p>Initial code from https://github.com/Microsoft/vscode-html-languageservice Initial copyright
 * Copyright (C) Microsoft Corporation. All rights reserved. Initial license: MIT
 *
...
public class DOMDocument extends DOMNode implements Document {
  ...
  public DOMDocument(
      TextDocument textDocument, URIResolverExtensionManager resolverExtensionManager) {
   ...
   *
   * @param textOffset
   * @return the element name on the left of the given position and null otherwise.
   */
   ...
   *
   * @return true if XML root element declares a "xsi:schemaLocation" and false otherwise.
   */
  ...
  /**
   * Returns true if XML root element declares a "xsi:noNamespaceSchemaLocation" and false
   * otherwise.
   *
   * @return true if XML root element declares a "xsi:noNamespaceSchemaLocation" and false
   *     otherwise.
   */
  ...
  /**
   * Initialize schemaLocation, noNamespaceSchemaLocation and hasNamespaces information if needed.
   */
    ...
    if (documentElement.hasAttributes()) {
          ...
          if (attributeName.equals(DOMAttr.XMLNS_ATTR)
            ...
            if (attributeValue != null && attributeValue.startsWith("http://www.w3.org/")) {
              if (attributeValue.endsWith("/XMLSchema-instance")) {
                schemaInstancePrefix =
                    attributeName.equals("xmlns") ? "" : getUnprefixedName(attributeName);
      ...
      if (schemaInstancePrefix != null) {
        ...
        noNamespaceSchemaLocation =
            createNoNamespaceSchemaLocation(documentElement, schemaInstancePrefix);
  ...
  /**
   * Returns true, if the given offset is before XML declaration ({@code <?xml ...?>}), false
   * otherwise.
   *
  ...
  private static NoNamespaceSchemaLocation createNoNamespaceSchemaLocation(
      DOMNode root, String schemaInstancePrefix) {
    DOMAttr attr =
        root.getAttributeNode(getPrefixedName(schemaInstancePrefix, "noNamespaceSchemaLocation"));
   ...
   *
   * @return true if XML document has a xml-model processing declaration and false otherwise.
   */
   ...
   *
   * @return the list of xml-model processing instruction declared in the document.
   */
  ...
  /**
   * Returns true if the document is bound to an external grammar (XML file associations, XLM
   * catalog) and false otherwise.
   *
   * @return true if the document is bound to an external grammar (XML file associations, XLM
   *     catalog) and false otherwise.
   */
   ...
   *
   * @return the external grammar location (XSD, DTD from xml file associations) and null otherwise.
   */
  ...
  /**
   * Returns the grammar location found by the namespace URI from the document root element (ex :
   * found with XML catalog) and null otherwise.
   *
   * @return the grammar location found by the namespace URI from the document root element (ex :
   *     found with XML catalog) and null otherwise.
   */
  ...
  private synchronized void intializeExternalGrammar() {
    ...
    try {
      if (resolverExtensionManager != null) {
        ...
        // location.
        try {
          externalGrammarLocation =
              resolverExtensionManager.getExternalGrammarLocation(new URI(getDocumentURI()));
        ...
        externalGrammarFromNamespaceURI =
            resolverExtensionManager.resolve(getDocumentURI(), namespaceURI, null);
      }
    } finally {
   ...
   */
  @Override
  public DOMNode renameNode(org.w3c.dom.Node n, String namespaceURI, String qualifiedName)
      throws DOMException {
    throw new UnsupportedOperationException();
  }
   ...
   *
   * @param elementName
   * @return the DTD Attribute list for the given element name and empty otherwise.
   */
  public Collection<DOMNode> findDTDAttrList(String elementName) {
    ...
    return docType.getChildren().stream()
        .filter(DOMNode::isDTDAttListDecl)
        .filter(n -> elementName.equals(((DTDAttlistDecl) n).getElementName()))
   ...
   *
   * @return the XML Schema prefix (ex : 'xs' for xmlns:xs="http://www.w3.org/2001/XMLSchema")
   */
