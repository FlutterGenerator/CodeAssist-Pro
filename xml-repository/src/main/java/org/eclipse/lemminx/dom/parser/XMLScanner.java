...
public class XMLScanner implements Scanner {
  ...
  private static final Predicate<Integer> START_ELEMENT_NAME_PREDICATE =
      ch -> {
        // ^[_:\w]
        return ch == _UDS || ch == _DDT || Character.isLetter(ch);
      };

  private static final Predicate<Integer> ELEMENT_NAME_PREDICATE =
      ch -> {
        // [_:\w-.\d]*
        return ch == _UDS /* '_' */
            || ch == _DDT /* ':' */
  ...
  private static final Predicate<Integer> ATTRIBUTE_NAME_PREDICATE =
      ch -> {
  ...
  private static final int[] END_WS_OR_PROLOG_PATTERN =
      new int[] {_NWL, _CAR, _WSP, _QMA, _RAN}; // \n or \r or ' '
  // or '?'

  private static final int[] START_COMMENT_PATTERN = new int[] {_EXL, _MIN, _MIN}; // !--

  private static final int[] START_CDATA_PATTERN =
      new int[] {_EXL, _OSB, _CVL, _DVL, _AVL, _TVL, _AVL, _OSB}; // !CDATA

  private static final int[] END_CDATA_PATTERN = new int[] {_CSB, _CSB, _RAN}; // ]]>

  private static final int[] START_DOCTYPE_PATTERN =
      new int[] {_EXL, _DVL, _OVL, _CVL, _TVL, _YVL, _PVL, _EVL}; // !DOCTYPE

  private static final int[] START_DTD_ELEMENT_PATTERN =
      new int[] {_EXL, _EVL, _LVL, _EVL, _MVL, _EVL, _NVL, _TVL}; // !ELEMENT

  private static final int[] START_DTD_ATTLIST_PATTERN =
      new int[] {_EXL, _AVL, _TVL, _TVL, _LVL, _IVL, _SVL, _TVL}; // !ATTLIST

  private static final int[] START_DTD_ENTITY_PATTERN =
      new int[] {_EXL, _EVL, _NVL, _TVL, _IVL, _TVL, _YVL}; // !ENTITY

  private static final int[] START_DTD_NOTATION_PATTERN =
      new int[] {_EXL, _NVL, _OVL, _TVL, _AVL, _TVL, _IVL, _OVL, _NVL}; // !NOTATION

  private static final int[] END_DTD_DOCTYPE_PATTERN = new int[] {_RAN, _LAN, _CSB}; // > || < || ]

  private static final int[] START_DTD_INTERNAL_SUBSET_PATTERN =
      new int[] {_OSB, _RAN, _LAN}; // [ | < | >
  ...
  boolean isDeclCompleted =
      false; // If any type of DTD declaration was supplied with all the required properties
  TokenType tempToken;
  boolean isDTDFile;
   ...
   *
   * <p>If the first attribute definition was completed in an ATTLIST declaration eg: <!ATTLIST unit
   * |power NMTOKEN #IMPLIED| << AFTER THIS IS COMPLETE, will be set to true description CDATA
   * #IMPLIED >
   */
  ...
  TokenType internalScan() {
    ...
    switch (state) {
      ...
      case PrologOrPI:
        ...
        if (stream.advanceUntilAnyOfChars(END_WS_OR_PROLOG_PATTERN)
            || stream.eos()) { // \n or \r or ' ' or '?'
      ...
      case AfterOpeningEndTag:
        ...
        if (stream.skipWhitespace()) { // white space is not valid here
          return finishToken(
              ...
              localize(
                  "error.unexpectedWhitespace", "Tag name must directly follow the open bracket."));
        }
        state = ScannerState.WithinEndTag;
      ...
      case DTDUnrecognizedParameters:
        ...
        if (stream.advanceUntilCharOrNewTag(_RAN)) { // >
          if (stream.peekChar() == _LAN) { // <
            state =
                isInsideDTDContent ? ScannerState.DTDWithinContent : ScannerState.DTDWithinDoctype;
          }
        }
        return finishToken(offset, TokenType.DTDUnrecognizedParameters);
      ...
      case DTDAfterEntityName:
        ...
        lastDoctypeKind =
            doctypeKind(); // eg: PUBLIC || SYSTEM, will advance the stream if either of these
        if (lastDoctypeKind.equals(DocumentTypeKind.PUBLIC.name())) {
  ...
  public static Scanner createScanner(String input, int initialOffset, boolean isDTDFile) {
    return createScanner(
        input,
        initialOffset,
        isDTDFile ? ScannerState.DTDWithinContent : ScannerState.WithinContent,
        isDTDFile);
  }
  ...
  public static Scanner createScanner(
      String input, int initialOffset, ScannerState initialState, boolean isDTDFile) {
