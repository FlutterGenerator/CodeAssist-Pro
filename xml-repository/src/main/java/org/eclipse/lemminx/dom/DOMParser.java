...
public class DOMParser {
  ...
  public DOMDocument parse(
      String text, String uri, URIResolverExtensionManager resolverExtensionManager) {
  ...
  public DOMDocument parse(
      TextDocument document, URIResolverExtensionManager resolverExtensionManager) {
    ...
    while (token != TokenType.EOS) {
      ...
      if (previousTokenWasEndTagOpen) {
        ...
        if (token == TokenType.EndTagClose) {
          ...
          while (!(curr.isElement() && ((DOMElement) curr).isSameTag(closeTag))
              && curr.parent != null) {
      ...
      switch (token) {
        case StartTagOpen:
          {
            ...
            DOMElement child =
                xmlDocument.createElement(scanner.getTokenOffset(), scanner.getTokenEnd());
        ...
        case StartTagClose:
          if (curr.isElement()) {
            ...
            if (element.hasTagName()
                && isEmptyElement(element.getTagName())
                && curr.parent != null) {
              curr.closed = true;
              curr = curr.parent;
            }
            ...
            if (element.getTarget() != null
                && isEmptyElement(element.getTarget())
                && curr.parent != null) {
              curr.closed = true;
              curr = curr.parent;
            }
        ...
        case EndTag:
          ...
          if (curr != xmlDocument) {
            ...
            DOMElement element =
                xmlDocument.createElement(scanner.getTokenOffset() - 2, scanner.getTokenEnd());
            element.endTagOpenOffset = endTagOpenOffset;
        ...
        case CDATATagOpen:
          {
            DOMCDATASection cdataNode =
                xmlDocument.createCDataSection(scanner.getTokenOffset(), text.length());
        ...
        case StartPrologOrPI:
          {
            DOMProcessingInstruction prologOrPINode =
                xmlDocument.createProcessingInstruction(scanner.getTokenOffset(), text.length());
        ...
        case DTDStartDoctypeTag:
          {
            DOMDocumentType doctype =
                xmlDocument.createDocumentType(scanner.getTokenOffset(), text.length());
        ...
        case DTDEndTag:
          {
            if ((curr.isDTDElementDecl()
                || curr.isDTDAttListDecl()
        ...
        case DTDUnrecognizedParameters:
          {
            ...
            node.setUnrecognized(
                scanner.getTokenOffset(), ((XMLScanner) scanner).getLastNonWhitespaceOffset());
            break;
          }
