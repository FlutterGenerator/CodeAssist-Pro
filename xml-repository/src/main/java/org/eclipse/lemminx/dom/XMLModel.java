...
public class XMLModel {
  ...
  static List<XMLModel> createXMLModels(DOMDocument document) {
    ...
    if (children != null && !children.isEmpty()) {
      return children.stream()
          .filter(XMLModel::isXMLModel)
          .map(node -> new XMLModel((DOMProcessingInstruction) node))
   ...
   * @return true if the given node is a xml-model processing instruction and false otherwise.
   */
  public static boolean isXMLModel(DOMNode node) {
    return node.isProcessingInstruction()
        && XML_MODEL_PI.equals(((ProcessingInstruction) node).getTarget());
  }
  ...
  public boolean isApplicable() {
    return isApplicableForDTD(getModelDeclaration())
        || isApplicableForXSD(getModelDeclaration())
