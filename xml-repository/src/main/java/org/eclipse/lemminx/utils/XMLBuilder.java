...
public class XMLBuilder {
  ...
  private XMLBuilder addAttribute(DOMAttr attr, int level, boolean surroundWithQuotes) {
    ...
    addAttributeContents(
        attr.getName(), attr.hasDelimiter(), attr.getOriginalValue(), surroundWithQuotes, attr);
    return this;
  }
  ...
  private void formatAttributeValue(
      String name, String valueWithoutQuote, Character quote, DOMAttr attr) {
      ...
      for (IFormatterParticipant formatterParticipant : formatterParticipants) {
        try {
          if (formatterParticipant.formatAttributeValue(
              name, valueWithoutQuote, quote, attr, this)) {
            return;
          }
        } catch (Exception e) {
          LOGGER.log(
