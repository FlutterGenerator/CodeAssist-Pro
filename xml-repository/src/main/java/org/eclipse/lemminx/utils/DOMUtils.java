...
public class DOMUtils {
  ...
  private static final String HTTP_WWW_W3_ORG_2001_XML_SCHEMA_NS =
      "http://www.w3.org/2001/XMLSchema";

  private static final String URN_OASIS_NAMES_TC_ENTITY_XMLNS_XML_CATALOG_NS =
  ...
  private static final String HTTP_WWW_W3_ORG_1999_XSL_TRANSFORM_NS =
   ...
   *
   * @return true if the XML document is a RelaxNG grammar XML syntax and false otherwise.
   */
   ...
   *
   * @param uri the URI to check
   * @return true if the given URI is a RelaxNG grammar XML syntax and false otherwise.
   */
   ...
   *
   * @param uri the URI to check
   * @return true if the given URI is a RelaxNG grammar compact syntax and false otherwise.
   */
  ...
  /**
   * Returns true if the document element root is bound to the given namespace and false otherwise.
   *
   * @param document
   * @param namespace
   * @return true if the document element root is bound to the given namespace and false otherwise.
   */
  ...
  public static boolean isDTD(String uri) {
    return uri != null
        && (uri.endsWith(DTD_EXTENSION)
            || uri.endsWith(ENT_EXTENSION)
            || uri.endsWith(MOD_EXTENSION));
  }
   ...
   */
  public static DOMDocument loadDocument(
      String documentURI, URIResolverExtensionManager resolverExtensionManager) {
    try {
      return DOMParser.getInstance()
          .parse(
              IOUtils.convertStreamToString(new URL(documentURI).openStream()),
              documentURI,
   ...
   *
   * @return an instance of SAX parser factory by disabling external entities declarations.
