...
public class XMLPositionUtility {
  ...
  private static final Predicate<Character> ENTITY_NAME_PREDICATE =
      ch -> {
        // [_:\w-.\d]*
  ...
  public static Range selectAttributeValueAt(
      String attrName, int offset, boolean withouQuote, DOMDocument document) {
  ...
  public static Range selectAttributeValueFromGivenValue(
      String attrValue, int offset, DOMDocument document) {
    DOMNode element = document.findNodeAt(offset);
  ...
  private static Range createAttrValueRange(
      DOMAttr attr, DOMDocument document, boolean withoutQuote) {
  ...
  public static Range selectAttributeValueByGivenValueAt(
      String attrValue, int offset, DOMDocument document) {
    DOMNode element = document.findNodeAt(offset);
    if (element != null && element.hasAttributes()) {
  ...
  public static Range selectAttributeNameFromGivenNameAt(
      String attrName, int offset, DOMDocument document) {
    DOMNode element = document.findNodeAt(offset);
    if (element != null && element.hasAttributes()) {
      DOMAttr attr = element.getAttributeNode(attrName);
      if (attr != null) {
   ...
   *
   * <p>For example, if attrName = "xsi:example", the range for "xsi" will be returned
   */
  public static Range selectAttributePrefixFromGivenNameAt(
      String attrName, int offset, DOMDocument document) {
    if (attrName == null) {
      return null;
    }
  ...
  public static Range selectAttributeFromGivenNameAt(
      String attrName, int offset, DOMDocument document) {
    DOMNode element = document.findNodeAt(offset);
    if (element != null && element.hasAttributes()) {
      DOMAttr attr = element.getAttributeNode(attrName);
   ...
   */
  public static Range selectChildNodeAttributeValueFromGivenNameAt(
      String childNodeName, String attrName, int offset, DOMDocument document) {
   ...
   * @return true if the given position is within an attribute value, and false otherwise
   */
  public static boolean isInAttributeValue(DOMDocument xmlDocument, Position position) {
    try {
      ...
      LOGGER.log(
          Level.SEVERE, "Bad range when checking if a position is within an attribute value", e);
      return false;
    }
  }

  /**
   * Finds the root element of the given document and returns the attribute value <code>Range</code>
   * for the attribute <code>attrName</code>.
   *
   * <p>If <code>attrName</code> is not declared then null is returned.
   *
   ...
   * @return The range in <code>document</code> where the declared value of attribute <code>attrName
   *     </code> resides (including quotations), or null if the attriubte is not declared.
   */
  ...
  public static Range selectChildEndTag(String childTag, int offset, DOMDocument document) {
    ...
    while (curr != null) {
      ...
      if (child == null) {
        ...
        return createRange(
            child.getStart() + 1, child.getStart() + 1 + childTag.length(), document);
      }
    }

    String parentName = ((DOMElement) parent).getTagName();
    return createRange(
        parent.getStart() + 2, parent.getStart() + 2 + parentName.length(), document);
  }
  ...
  private static DOMNode findUnclosedChildNode(String childTag, List<DOMNode> children) {
    for (DOMNode child : children) {
      if (child.isElement()
          && childTag != null
          && childTag.equals(((DOMElement) child).getTagName())
          && !child.isClosed()) {
        return child;
      }
    }
  ...
  /**
   * Returns the range of the root start tag (excludes the '<') of the given <code>document</code>
   * and null otherwise.
   *
   * @param document the DOM document.
   * @return the range of the root start tag (excludes the '<') of the given <code>document</code>
   *     and null otherwise.
   */
  ...
  /**
   * Returns the range of the start tag name (excludes the '<') of the given <code>element</code>
   * and null otherwise.
   *
   * @param element the DOM element
   * @return the range of the start tag of the given <code>element</code> and null otherwise.
   */
  ...
  /**
   * Returns the range of a tag's local name. If the tag does not have a prefix, implying it doesn't
   * have a local name, it will return null.
   *
   * @param element
   * @return
   */
   ...
   * and null otherwise.
   *
   * <p>If suffixOnly is true then it will try to return the range of the localName/suffix. Else it
   * will return null.
   *
   * @param element the DOM element
  ...
  /**
   * Returns the range of the end tag of the given <code>element</code> name and null otherwise.
   *
   * @param element the DOM element
   * @return the range of the end tag of the given <code>element</code> and null otherwise.
   */
  ...
  /**
   * Returns the range of the end tag of the given <code>element</code> and null otherwise.
   *
   * @param element the DOM element
   * @return the range of the end tag of the given <code>element</code> and null otherwise.
   */
   ...
   * @return the range of the entity reference in a text node (ex : &amp;) and null otherwise.
   */
   ...
   * @param offset the offset
   * @param document the document
   * @param endsWithSemicolon true if the entity reference must end with ';' and false otherwise.
   * @return the range of the entity reference in a text node (ex : &amp;) and null otherwise.
   */
  public static EntityReferenceRange selectEntityReference(
      int offset, DOMDocument document, boolean endsWithSemicolon) {
    String text = document.getText();
    ...
    String name =
        endsWithSemicolon
            ? document.getText().substring(entityReferenceStart + 1, entityReferenceEnd - 1)
            : null;
    return new EntityReferenceRange(
        name, createRange(entityReferenceStart, entityReferenceEnd, document));
  }

  /**
   * Returns the start offset of the entity reference (ex : &am|p;) from the left of the given
   * offset and -1 if no entity reference.
   *
   * @param text the XML content.
   * @param offset the offset.
   * @return the start offset of the entity reference (ex : &am|p;) from the left of the given
   *     offset and -1 if no entity reference.
   */
  ...
  /**
   * Returns the end offset of the entity reference (ex : &am|p;) from the right of the given offset
   * and -1 if no entity reference.
   *
   * @param text the XML content.
   * @param offset the offset.
   * @return the end offset of the entity reference (ex : &am|p;) from the right of the given offset
   *     and -1 if no entity reference.
   */
    ...
    if (node != null) {
      if (node.isElement()) {
        ...
        if (node.hasChildNodes()) {
          return createRange(
              element.getStartTagCloseOffset() + 1, element.getEndTagOpenOffset(), document);
        }
   ...
   *
   * <p>This method assumes that the node located at offset only contains text.
   *
   * <p>For example, if the node located at offset is:
   *
   * <p><a> hello
   *
   * <p></a>
   *
   * <p>the returned range will cover only "hello".
   *
   * @param offset
   * @param document
   * @return range covering the trimmed text belonging to the node located at offset
   */
  ...
  /**
   * Will give the range for the last VALID DTD Decl parameter at 'offset'. An unrecognized
   * Parameter is not considered VALID,
   *
   * <p>eg: "<!ELEMENT elementName (content) UNRECOGNIZED_CONTENT" will give the range of 1
   * character length, after '(content)'
   */
  public static Range getLastValidDTDDeclParameter(
      int offset, DOMDocument document, boolean selectWholeParameter) {
    DOMNode node = document.findNodeAt(offset);
   ...
   * Parameter is not considered VALID,
   *
   * <p>eg: <!ELEMENT elementName (content) UNRECOGNIZED_CONTENT will give the range 1 character
   * after '(content)'
   */
  ...
  /**
   * Will give the range for the last DTD Decl parameter at 'offset'. An unrecognized Parameter is
   * considered as well.
   *
   * <p>eg: <!ELEMENT elementName (content) UNRECOGNIZED_CONTENT will give the range 1 character
   * after '(content)'
  ...
  /**
   * Returns the location link for the given <code>origin</code> and <code>target</code> nodes.
   *
   * @param origin the origin node.
   * @param target the target node.
   * @return the location link for the given <code>origin</code> and <code>target</code> nodes.
   */
  ...
  public static LocationLink createLocationLink(Range origin, DOMRange target) {
    ...
    return new LocationLink(
        targetDocument.getDocumentURI(), targetRange, targetSelectionRange, origin);
  }
  ...
  public static Location createLocation(DOMRange target) {
    DOMDocument targetDocument = target.getOwnerDocument();
    Range targetRange =
        XMLPositionUtility.createRange(target.getStart(), target.getEnd(), targetDocument);
   ...
   * @param adjust <code>true</code> means the first and last character of <code>target</code> will
   *     not be a part of the link.
   * @return A DocumentLink over the range <code>target</code> that points to <code>location</code>.
   * @throws BadLocationException if <code>target</code> is out of the bounds of the document
   */
   ...
   *
   * <p>Returns null if node is not a DOMElement, or if a node does not exist at offset.
   *
   * @param offset
   * @param document
  ...
  public static boolean doesTagCoverPosition(
      Range startTagRange, Range endTagRange, Position position) {
  ...
  public static Range getTagNameRange(
      TokenType tokenType, int startOffset, DOMDocument xmlDocument) {
    ...
    if (token != TokenType.EOS) {
      try {
        return new Range(
            xmlDocument.positionAt(scanner.getTokenOffset()),
            xmlDocument.positionAt(scanner.getTokenEnd()));
      } catch (BadLocationException e) {
        LOGGER.log(
            Level.SEVERE,
            "While creating Range in XMLHighlighting the Scanner's Offset was a BadLocation",
            e);
        return null;
      }
    }
   ...
   *
   * <p>For the given attr value:
   *
   * <p><element attr="value" /> it will return <element attr="|value|" /> range without ".
   *
