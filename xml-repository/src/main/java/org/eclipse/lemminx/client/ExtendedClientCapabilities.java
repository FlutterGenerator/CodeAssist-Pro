...
public class ExtendedClientCapabilities {
   ...
   *
   * <p>See {@link org.eclipse.lemminx.customservice.ActionableNotification} and {@link
   * org.eclipse.lemminx.customservice.XMLLanguageClientAPI#actionableNotification}
   *
   ...
   *
   * <p>See {@link org.eclipse.lemminx.client.ClientCommands#OPEN_SETTINGS}
   *
  ...
  /**
   * Returns true if the client supports the `xml.open.binding.wizard` command using dropdown and
   * false otherwise
   *
  ...
  /**
   * Sets the boolean permitting language server to exit on client shutdown() request, without
   * waiting for client to call exit()
   *
  ...
  /**
   * Returns true if the client should exit on shutdown() request and avoid waiting for an exit()
   * request
   *
