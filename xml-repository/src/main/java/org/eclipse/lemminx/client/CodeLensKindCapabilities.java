...
public class CodeLensKindCapabilities {
  /**
   * The codeLens kind values the client supports. When this property exists the client also
   * guarantees that it will handle values outside its set gracefully and falls back to a default
   * value when unknown.
   *
   * <p>If this property is not present the client only supports the codeLens kinds from `File` to
