...
public class CloseTagCodeAction implements ICodeActionParticipant {
  ...
  public void doCodeAction(
    ...
    try {
      ...
      if (element.hasEndTag() && !element.isEndTagClosed() && element.hasTagName()) {
        ...
        if (!startTagClosed) {
          doCodeActionsForStartTagUnclosed(
              element, document, diagnosticRange, diagnostic, codeActions);
        } else {
          doCodeActionsForStartTagClosed(
              element, document, diagnosticRange, diagnostic, codeActions);
        }
      }
   ...
   */
  private void doCodeActionsForEndTagUnclosed(
      DOMElement element, DOMDocument document, Diagnostic diagnostic, List<CodeAction> codeActions)
      throws BadLocationException {
    // ex: <foo> </foo
    Position endTagPosition =
        document.positionAt(element.getEndTagOpenOffset() + 2 + element.getTagName().length());
    // Close with '>'
    CodeAction autoCloseAction =
        CodeActionFactory.insert(
            "Close end-tag with '>'", endTagPosition, ">", document.getTextDocument(), diagnostic);
   ...
   * @throws BadLocationException
   */
  private void doCodeActionsForStartTagUnclosed(
      DOMElement element,
    ...
    if (!element.hasEndTag()) {
      ...
      if (hasSlash) {
        // ex : <foo /
        CodeAction autoCloseAction =
            insertGreaterThanCharacterCodeAction(document, diagnostic, closeAngleBracketPosition);
        codeActions.add(autoCloseAction);
      } else {
        // ex : <foo
        // ex : <foo attr="
        ...
        if (tagName != null) {
          ...
          CodeAction closeEndTagAction =
              CodeActionFactory.insert(
                  "Close with '" + insertText + "'",
                  closeAngleBracketPosition,
                  insertText,
                  document.getTextDocument(),
                  diagnostic);
   ...
   * @throws BadLocationException
   */
  private void doCodeActionsForStartTagClosed(
      DOMElement element,
      DOMDocument document,
      Range diagnosticRange,
      Diagnostic diagnostic,
      ...
      if (!element.hasChildNodes()) {
        ...
        for (DOMNode child : children) {
          if (child.isElement()) {
            ...
            if (!childElement.hasTagName() || childElement.isOrphanEndTag()) {
              ...
              CodeAction replaceTagAction =
                  replaceEndTagNameCodeAction(childElement, replaceTagName, diagnostic);
        ...
        if (!hasChildWithNoTagName) {
          ...
          if (hasElements(element)) {
            ...
            LineIndentInfo indentInfo =
                document.getLineIndentInfo(diagnosticRange.getStart().getLine());
            insertText =
                indentInfo.getLineDelimiter() + indentInfo.getWhitespacesIndent() + insertText;
          }
   ...
   */
  private static CodeAction insertGreaterThanCharacterCodeAction(
      DOMDocument document, Diagnostic diagnostic, Position position) {
    CodeAction autoCloseAction =
        CodeActionFactory.insert(
            "Close with '>'", position, ">", document.getTextDocument(), diagnostic);
   ...
   * @throws BadLocationException
   */
  private static CodeAction removeTagCodeAction(
      DOMElement element, DOMDocument document, Diagnostic diagnostic) throws BadLocationException {
    String text = document.getText();
    ...
    CodeAction removeAction =
        CodeActionFactory.remove(
  ...
  /**
   * Create a code action which replaces the end tag name of the given element with the given
   * replace tag name.
   *
   * @param element the DOM element to replace
   * @param replaceTagName the replace tag name
   * @param diagnostic the diagnostic.
   * @return a code action which replaces the end tag name of the given element with the given
   *     replace tag name.
   */
  private static CodeAction replaceEndTagNameCodeAction(
      DOMElement element, String replaceTagName, Diagnostic diagnostic) {
    // <a><b></c>
    ...
    return CodeActionFactory.replace(
        "Replace '" + tagName + "' with '" + replaceTagName + "' closing tag",
        replaceRange,
        replaceText,
        document.getTextDocument(),
        diagnostic);
  ...
  /**
   * Returns true if the given element has elements as children and false otherwise.
   *
   * @param element the DOM element.
   * @return true if the given element has elements as children and false otherwise.
   */
