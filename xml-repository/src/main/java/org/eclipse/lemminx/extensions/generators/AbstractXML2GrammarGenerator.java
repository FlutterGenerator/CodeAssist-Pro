 ...
 *
 * <p>The process is to build a generic {@link Grammar} instance from the XML source and each
 * implementation uses this grammar information to generate the proper grammar.
 *
...
public abstract class AbstractXML2GrammarGenerator<T extends FileContentGeneratorSettings>
  ...
  public String generate(
    ...
    String newText =
        doGenerate(prototypeDocument, sharedSettings, generatorSettings, cancelChecker);
    if (formatter == null) {
      return newText;
    }
    // Format the generated grammar.
    return formatter.formatFull(
        newText, "grammar." + getFileExtension(), sharedSettings, cancelChecker);
  }
   ...
   *
   * <ul>
   *   <li>flat=true: helpful for DTD which declares <!ELEMENT without hierarchy.
   *   <li>flat=false: helpful for XSD which declares xs:element with hierarchy.
   * </ul>
   *
   * @return true if element declaration must be stored as flat mode and false otherwise.
   */
  ...
  protected abstract void generate(
      Grammar grammar, T grammarSettings, XMLBuilder out, CancelChecker cancelChecker);
  ...
  private static Grammar createGrammar(
      Document sourceDocument, boolean flat, CancelChecker cancelChecker) {
   ...
   */
  private static ElementDeclaration getElementDecl(
      Grammar grammar, ContainerDeclaration container, boolean flat, Element element) {
