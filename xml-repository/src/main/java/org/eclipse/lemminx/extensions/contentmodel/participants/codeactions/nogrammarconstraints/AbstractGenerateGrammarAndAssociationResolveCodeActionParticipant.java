 ...
 *
 * <ul>
 *   <li>insert the grammar association in the XML (ex : xsi:noNamespaceSchemaLocation)
 *   <li>generate the grammar XSD/DTD file
 * </ul>
 *
 * @author Angelo ZERR
 */
public abstract class AbstractGenerateGrammarAndAssociationResolveCodeActionParticipant
  ...
  private static final Logger LOGGER =
      Logger.getLogger(
          AbstractGenerateGrammarAndAssociationResolveCodeActionParticipant.class.getName());

  @Override
  public final CodeAction resolveCodeAction(
      ICodeActionResolverRequest request, CancelChecker cancelChecker) {
    try {
      ...
      TextDocumentEdit noNamespaceSchemaLocationEdit =
          createFileEdit(grammarFileName, document, sharedSettings);
      ...
      String schemaTemplate =
          generator.generate(
              document, sharedSettings, getFileContentGeneratorSettings(), cancelChecker);

      // Update the unresolved code action
      updateGrammarFileAndBindIt(
          resolved, grammarURI, schemaTemplate, noNamespaceSchemaLocationEdit);
      return resolved;
  ...
  private static CodeAction updateGrammarFileAndBindIt(
      CodeAction codeAction, String grammarURI, String grammarContent, TextDocumentEdit boundEdit) {
    WorkspaceEdit createAndAddContentEdit =
        CodeActionFactory.createFileEdit(grammarURI, grammarContent);
    codeAction.setEdit(createAndAddContentEdit);
    codeAction.getEdit().getDocumentChanges().add(Either.forLeft(boundEdit));
    return codeAction;
  }

  protected abstract TextDocumentEdit createFileEdit(
      String grammarFileName, DOMDocument document, SharedSettings sharedSettings)
