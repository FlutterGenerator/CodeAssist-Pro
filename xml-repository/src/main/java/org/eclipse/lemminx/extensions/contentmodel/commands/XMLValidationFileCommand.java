 ...
 *
 * <ul>
 *   <li>remove the referenced grammar in the XML file from the Xerces grammar pool (used by the
 *       Xerces validation) and the content model documents cache (used by the XML completion/hover
 *       based on the grammar)
 *   <li>trigger the validation for the given XML file
 * </ul>
 *
 * @author Angelo ZERR
 */
public class XMLValidationFileCommand extends AbstractDOMDocumentCommandHandler {
  ...
  public XMLValidationFileCommand(
  ...
  protected Object executeCommand(
    ...
    // force do download.
    JsonObject validationArgs =
        params.getArguments().size() > 1 ? (JsonObject) params.getArguments().get(1) : null;
