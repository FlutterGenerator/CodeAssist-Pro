...
public class FilePathCompletionResult {
  ...
  private static final Predicate<Character> isStartValidCharForSimplePath =
      (c) -> c != '/' && c != '\\';
  ...
  /**
   * Returns the start offset of the last path declared (ex : <a href="path/to/|file.xml").
   *
  ...
  /**
   * Returns the end offset of the last path declared (ex : <a href="path/to/file.xml|").
   *
   ...
   *
   * <p>Ex : <a href="path/to/file.xml" />
   *
   * <p>the method will return 'file://C:/path/to'
   *
   ...
   * @param separator the separator used to declare multiple files and null otherwise.
   * @return the file path completion result.
   */
  public static FilePathCompletionResult create(
      String content,
      String fileUri,
      int startNodeOffset,
      int endNodeOffset,
    ...
    if (isMultiFilePath) {
      ...
      endPathOffset =
          StringUtils.findEndWord(
              content, completionOffset - 1, endNodeOffset, c -> c != separator);
      if (endPathOffset == -1) {
        endPathOffset = endNodeOffset;
      }
    }
    int startPathOffset =
        StringUtils.findStartWord(content, completionOffset, startNodeOffset, isStartValidChar);
    int startBaseDirOffset = startNodeOffset;
    if (isMultiFilePath && !isStartValidChar.test(content.charAt(startPathOffset - 1))) {
      // multiple file path (ex : <a hrefs="path/to/file1.xml;path/to/file2.xml" />
      int tmp =
          StringUtils.findStartWord(
              content, completionOffset, startNodeOffset, c -> c != separator);
      if (tmp != -1) {
  ...
  private static boolean isAbsoluteWindowsPath(String currentText) {
    ...
    return currentText.charAt(1) == ':'
        && (currentText.charAt(2) == '\\' || currentText.charAt(2) == '/');
  }
}
