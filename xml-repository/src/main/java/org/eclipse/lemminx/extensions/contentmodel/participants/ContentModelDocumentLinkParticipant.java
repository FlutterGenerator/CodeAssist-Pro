 ...
 *
 * <ul>
 *   <li>XML Schema xsi:noNamespaceSchemaLocation
 *   <li>DTD SYSTEM (ex : <!DOCTYPE root-element SYSTEM "./extended.dtd" )
 *   <li>Entity SYSTEM (ex : <!ENTITY % xinclude SYSTEM
 *       "http://www.docbook.org/xml/4.4/xinclude.mod"> )
 *   <li>XML Schema xsi:schemaLocation
 * </ul>
 *
 * @author Angelo ZERR
 */
public class ContentModelDocumentLinkParticipant implements IDocumentLinkParticipant {
  ...
  public void findDocumentLinks(DOMDocument document, List<DocumentLink> links) {
    ...
    if (docType != null) {
      ...
      String location =
          resolverManager.resolve(
              ...
              docType.getPublicIdWithoutQuotes(),
      ...
      for (int i = 0; i < entities.getLength(); i++) {
        ...
        location =
            resolverManager.resolve(
                document.getDocumentURI(), entity.getPublicId(), entity.getSystemId());
        if (location != null) {
          try {
    ...
    for (XMLModel xmlModel : xmlModels) {
      String location =
          resolverManager.resolve(document.getDocumentURI(), null, xmlModel.getHref());
      if (location != null) {
        try {
    ...
    if (noNamespaceSchemaLocation != null) {
      try {
        String location =
            resolverManager.resolve(
                document.getDocumentURI(), null, noNamespaceSchemaLocation.getLocation());
        if (location != null) {
    ...
    if (schemaLocation != null) {
      try {
        Collection<SchemaLocationHint> schemaLocationHints =
            schemaLocation.getSchemaLocationHints();
        String location;
        for (SchemaLocationHint schemaLocationHint : schemaLocationHints) {
          location =
              resolverManager.resolve(
                  document.getDocumentURI(), null, schemaLocationHint.getHint());
          if (location != null) {
