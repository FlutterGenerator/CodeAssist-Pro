...
public class DTDUtils {
   ...
   * @param matchName true if the origin name must match the value of target name and false
   *     otherwise.
   * @param collector collector to collect DTD <!ELEMENT target name.
   */
  public static void searchDTDTargetElementDecl(
      DTDDeclParameter originNameNode, boolean matchName, Consumer<DTDDeclParameter> collector) {
  ...
  /**
   * Search origin DTD node (<!ATTRLIST element-name) or (child <!ELEMENT element-name (child)) from
   * the given target DTD node (<!ELEMENT element-name.
   *
   ...
   */
  public static void searchDTDOriginElementDecls(
      DTDDeclNode targetNode,
            ...
            switch (origin.getNodeType()) {
              ...
              case DOMNode.DTD_ATT_LIST_NODE:
                ...
                if (name.equals(originAttribute.getElementName())) {
                  ...
                  collector.accept(
                      originAttribute.getNameParameter(), targetElement.getNameParameter());
                }
                break;
            }
          }
  ...
  private static void updateTracker(
      DTDGrammar grammar, Set<DTDGrammar> trackedGrammars, FilesChangedTracker tracker) {
