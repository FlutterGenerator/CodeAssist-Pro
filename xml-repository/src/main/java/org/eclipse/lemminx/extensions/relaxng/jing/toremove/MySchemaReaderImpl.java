...
/**
 * This class is a copy of {@link SchemaReaderImpl} adapted for LemMinx. This class could be removed
 * once issues
 *
 * <ul>
 ...
 * will be fixed.
 *
 * <p>See comment with ' UPDATED by LemMinx'
 *
 * @author Angelo ZERR
 */
public abstract class MySchemaReaderImpl extends AbstractSchemaReader {

  public static final PropertyId<SchemaPatternBuilder> SCHEMA_PATTERN_BUILDER =
      PropertyId.newInstance("SCHEMA_PATTERN_BUILDER", SchemaPatternBuilder.class);
  ...
  public Schema createSchema(SAXSource source, PropertyMap properties)
    ...
    if (dlf == null) dlf = new DatatypeLibraryLoader();
    try {
      Pattern start =
          SchemaBuilderImpl.parse(
  ...
  private static class SimplifiedSchemaPropertyMap implements PropertyMap {
  ...
  static Schema wrapPattern(Pattern start, SchemaPatternBuilder spb, PropertyMap properties)
    ...
    if (properties.contains(RngProperty.FEASIBLE)) start = FeasibleTransform.transform(spb, start);
    properties =
        new SimplifiedSchemaPropertyMap(
            AbstractSchema.filterProperties(properties, supportedPropertyIds), start);
    // UPDATED by LemMinx
    ...
    if (spb.hasIdTypes() && properties.contains(RngProperty.CHECK_ID_IDREF)) {
      ...
      if (idTypeMap == null) throw new IncorrectSchemaException();
      Schema idSchema;
      if (properties.contains(RngProperty.FEASIBLE))
        idSchema = new FeasibleIdTypeMapSchema(idTypeMap, properties);
      else idSchema = new IdTypeMapSchema(idTypeMap, properties);
      // UPDATED by LemMinx
