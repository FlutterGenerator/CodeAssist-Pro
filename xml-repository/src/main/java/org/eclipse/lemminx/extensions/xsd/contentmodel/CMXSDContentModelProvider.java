...
public class CMXSDContentModelProvider implements ContentModelProvider {
  ...
  private static final String XSI_NO_NAMESPACE_SCHEMA_LOCATION_BINDING_KIND =
  ...
  public Collection<Identifier> getIdentifiers(DOMDocument xmlDocument, String namespaceURI) {
    ...
    if (schemaLocation != null) {
      if (namespaceURI == null) {
        for (SchemaLocationHint locationHint : schemaLocation.getSchemaLocationHints()) {
          ...
          if (!StringUtils.isEmpty(location)) {
            identifiers.add(
                new Identifier(null, location, locationHint, XSI_SCHEMA_LOCATION_BINDING_KIND));
          }
        }
      } else {
      ...
      NoNamespaceSchemaLocation noNamespaceSchemaLocation =
          xmlDocument.getNoNamespaceSchemaLocation();
          ...
          if (!StringUtils.isEmpty(location)) {
            identifiers.add(
                new Identifier(
                    null,
                    location,
                    noNamespaceSchemaLocation.getAttr().getNodeAttrValue(),
  ...
  @Override
  public CMDocument createInternalCMDocument(
  ...
  public XSLoaderImpl getLoader() {
    ...
    loader.setParameter(
        "http://apache.org/xml/properties/internal/entity-resolver", resolverExtensionManager);
    loader.setParameter("http://apache.org/xml/properties/internal/entity-manager", entityManager);
    loader.setParameter(
        Constants.DOM_ERROR_HANDLER,
        new DOMErrorHandler() {
