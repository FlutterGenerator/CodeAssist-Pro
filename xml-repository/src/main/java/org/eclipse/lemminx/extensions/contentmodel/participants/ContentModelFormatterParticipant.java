...
/**
 * Formatter participant which uses XSD/DTD grammar information to know the {@link
 * FormatElementCategory} of a given element.
 *
 * <p>This participant is enabled when 'xml.format.grammarAwareFormatting' setting is set to true.
 *
 * @author Angelo ZERR
 */
    ...
    if (cmDocuments == null) {
      try {
        ...
        try {
          ...
          // If it takes longer than 2 seconds to download the schema, we should give up and proceed
          // with formatting
          return null;
        }
      }
