...
public class XMLReferencesDiagnosticParticipant implements IDiagnosticsParticipant {
  ...
  private static final String UNDEFINED_REFERENCE_MESSSAGE =
      "Undefined reference ''{0}'': nothing that matches the expression ''{1}'' defines ''{2}''.";

  private static final String INVALID_PREFIX_MESSSAGE =
      "Invalid reference ''{0}'': references to declarations that match the expression ''{1}''"
          + " require the ''{2}'' prefix.";
  ...
  public void doDiagnostics(
    ...
    Collection<ReferenceLink> links =
        SearchEngine.getInstance()
            .searchLinks(document, plugin.getReferencesSettings(), cancelChecker);
    if (links.isEmpty()) {
      return;
    }
          ...
          if (from.getValidationStatus() == ValidationStatus.INVALID_PREFIX) {
            ...
            String message =
                MessageFormat.format(
                    INVALID_PREFIX_MESSSAGE,
                    from.getValue(null),
                    link.getExpression().getTo(),
                    link.getExpression().getPrefix());
