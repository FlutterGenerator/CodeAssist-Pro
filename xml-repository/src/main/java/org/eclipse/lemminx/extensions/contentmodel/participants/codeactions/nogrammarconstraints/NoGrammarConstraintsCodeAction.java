...
public class NoGrammarConstraintsCodeAction implements ICodeActionParticipant {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(NoGrammarConstraintsCodeAction.class.getName());
  ...
  public NoGrammarConstraintsCodeAction() {
    ...
    resolveCodeActionParticipants.put(
        GenerateXSINoNamespaceSchemaCodeActionResolver.PARTICIPANT_ID,
        new GenerateXSINoNamespaceSchemaCodeActionResolver());
    resolveCodeActionParticipants.put(
        GenerateXMLModelWithXSDCodeActionResolver.PARTICIPANT_ID,
        new GenerateXMLModelWithXSDCodeActionResolver());
    resolveCodeActionParticipants.put(
        GenerateDocTypeCodeActionResolver.PARTICIPANT_ID, new GenerateDocTypeCodeActionResolver());
    resolveCodeActionParticipants.put(
        GenerateXMLModelWithDTDCodeActionResolver.PARTICIPANT_ID,
  ...
  @Override
  public void doCodeAction(
      ICodeActionRequest request, List<CodeAction> codeActions, CancelChecker cancelChecker) {
    ...
    try {
      ...
      if (!request.canSupportResolve()) {
        ...
        FileContentGeneratorManager generator =
            request.getComponent(FileContentGeneratorManager.class);
        schemaTemplate =
            generator.generate(
                document, sharedSettings, new XMLSchemaGeneratorSettings(), cancelChecker);
      }

      // xsi:noNamespaceSchemaLocation
      // Create code action to create the XSD file with the generated XSD content
      CodeAction noNamespaceSchemaLocationAction =
          createNoNamespaceSchemaLocationCodeAction(
              schemaURI, schemaFileName, schemaTemplate, request, cancelChecker);
      codeActions.add(noNamespaceSchemaLocationAction);

      // xml-model
      CodeAction xsdWithXmlModelAction =
          createXmlModelCodeAction(
              schemaURI,
              schemaFileName,
              schemaTemplate,
              GenerateXMLModelWithXSDCodeActionResolver.PARTICIPANT_ID,
      ...
      if (!request.canSupportResolve()) {
        ...
        FileContentGeneratorManager generator =
            request.getComponent(FileContentGeneratorManager.class);
        dtdTemplate =
            generator.generate(document, sharedSettings, new DTDGeneratorSettings(), cancelChecker);
      }

      // <!DOCTYPE ${1:root-element} SYSTEM \"${2:file}.dtd\">
      CodeAction docTypeAction =
          createDocTypeCodeAction(dtdURI, dtdFileName, dtdTemplate, request, cancelChecker);
      codeActions.add(docTypeAction);

      // xml-model
      CodeAction dtdWithXmlModelAction =
          createXmlModelCodeAction(
              dtdURI,
              dtdFileName,
              dtdTemplate,
      ...
      if (!request.canSupportResolve()) {
        ...
        rngTemplate =
            generator.generate(
                document, sharedSettings, new RelaxNGGeneratorSettings(), cancelChecker);
      }

      // xml-model
      CodeAction relaxNGWithXmlModelAction =
          createRelaxNGCodeAction(rngURI, rngFileName, rngTemplate, request, cancelChecker);
      ...
      if (sharedSettings.isBindingWizardSupport()) {
        ...
        CodeAction bindWithExistingGrammar =
            CodeActionFactory.createCommand(title, OPEN_BINDING_WIZARD, commandParams, diagnostic);
  ...
  private static CodeAction createNoNamespaceSchemaLocationCodeAction(
      String schemaURI,
    ...
    if (request.canSupportResolve()) {
      return createGenerateFileUnresolvedCodeAction(
          schemaURI,
          title,
          diagnostic,
          document,
      ...
      TextDocumentEdit noNamespaceSchemaLocationEdit =
          createXSINoNamespaceSchemaLocationEdit(
              schemaFileName, document, request.getSharedSettings());
      return createGrammarFileAndBindIt(
          title, schemaURI, schemaTemplate, noNamespaceSchemaLocationEdit, diagnostic);
    }
  }

  private static CodeAction createXmlModelCodeAction(
      String fileURI,
      String fileName,
      String fileTemplate,
    ...
    if (request.canSupportResolve()) {
      return createGenerateFileUnresolvedCodeAction(
          fileURI, title, diagnostic, document, participantId);
    }
    SharedSettings sharedSettings = request.getSharedSettings();
    TextDocumentEdit xsdWithXMLModelEdit =
        createXmlModelEdit(fileName, null, document, sharedSettings);
    CodeAction xsdWithXmlModelAction =
        createGrammarFileAndBindIt(title, fileURI, fileTemplate, xsdWithXMLModelEdit, diagnostic);
    return xsdWithXmlModelAction;
  }

  private CodeAction createDocTypeCodeAction(
      String dtdURI,
      String dtdFileName,
      String dtdTemplate,
      ICodeActionRequest request,
    ...
    if (request.canSupportResolve()) {
      return createGenerateFileUnresolvedCodeAction(
          dtdURI, title, diagnostic, document, GenerateDocTypeCodeActionResolver.PARTICIPANT_ID);
    }
    SharedSettings sharedSettings = request.getSharedSettings();
  ...
  private CodeAction createRelaxNGCodeAction(
      String rngURI,
      String rngFileName,
      String rngTemplate,
      ICodeActionRequest request,
    ...
    SharedSettings sharedSettings = request.getSharedSettings();
    TextDocumentEdit relaxNGWithXMLModelEdit =
        createXmlModelEdit(rngFileName, null, document, sharedSettings);
    return createGrammarFileAndBindIt(
        title, rngURI, rngTemplate, relaxNGWithXMLModelEdit, diagnostic);
  }

  private static CodeAction createGenerateFileUnresolvedCodeAction(
      String generateFileURI,
      String title,
  ...
  private static CodeAction createGrammarFileAndBindIt(
      String title,
      String grammarURI,
      String grammarContent,
      TextDocumentEdit boundEdit,
      Diagnostic diagnostic) {
    CodeAction codeAction =
        CodeActionFactory.createFile(title, grammarURI, grammarContent, diagnostic);
    codeAction.getEdit().getDocumentChanges().add(Either.forLeft(boundEdit));
    return codeAction;
  }

  public static TextDocumentEdit createXSINoNamespaceSchemaLocationEdit(
      String schemaFileName, DOMDocument document, SharedSettings sharedSettings)
      throws BadLocationException {
    String delimiter = document.getTextDocument().lineDelimiter(0);
    DOMElement documentElement = document.getDocumentElement();
    int beforeTagOffset =
        documentElement != null
            ? documentElement.getStartTagOpenOffset()
            : document.getLastChild() != null ? document.getLastChild().getEnd() : 0;
    int afterTagOffset =
        documentElement != null
            ? beforeTagOffset + 1 + documentElement.getTagName().length()
            : beforeTagOffset;
    ...
    return CodeActionFactory.insertEdit(
        insertText.toString(), position, document.getTextDocument());
  }

  public static TextDocumentEdit createXSISchemaLocationEdit(
      String schemaFileName,
  ...
  public static TextDocumentEdit createXmlModelEdit(
    ...
    String xmlModelInsertText =
        (documentElement == null && document.getLastChild() != null ? delimiter : "")
    ...
    if (documentElement != null && StringUtils.isEmpty(targetNamespace)) {
      return CodeActionFactory.insertEdit(
          xmlModelInsertText, xmlModelPosition, document.getTextDocument());
    }
    ...
    int afterTagOffset =
        beforeTagOffset + (documentElement != null ? 1 + documentElement.getTagName().length() : 0);
    Position xmlNamespacePosition = document.positionAt(afterTagOffset);

    return CodeActionFactory.insertEdits(
  ...
  public static TextDocumentEdit createDocTypeEdit(
      String dtdFileName, DOMDocument document, SharedSettings sharedSettings)
      throws BadLocationException {
    String delimiter = document.getTextDocument().lineDelimiter(0);
    DOMElement documentElement = document.getDocumentElement();
    int beforeTagOffset =
