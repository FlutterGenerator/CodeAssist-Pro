 ...
 *
 * <ul>
 *   <li>disable only DTD validation if required
 *   <li>disable doctype declaration according validation settings
 *   <li>disable external entities according validation settings
 *   <li>manage a custom grammar pool to retrieve compiled XML Schema/DTD from a given XML file path
 * </ul>
 */
class LSPXMLParserConfiguration extends XMLModelAwareParserConfiguration {
  ...
  public LSPXMLParserConfiguration(
      ...
      boolean disableDTDValidation,
      LSPErrorReporterForXML reporterForXML,
      LSPErrorReporterForXML reporterForGrammar,
      LSPXMLEntityManager entityManager,
    ...
    boolean disallowDocTypeDecl =
        validationSettings != null ? validationSettings.isDisallowDocTypeDecl() : false;
    ...
    super.setFeature(
        "http://xml.org/sax/features/external-general-entities", resolveExternalEntities);
    super.setFeature(
        "http://xml.org/sax/features/external-parameter-entities", resolveExternalEntities);
    // Enable xi:include validation if settings is set to true.
    boolean isXIncludeEnabled =
        validationSettings != null ? validationSettings.getXInclude().isEnabled() : false;
  ...
  private void disableDTDValidation() {
    ...
    if (validator != null) {
      ...
      // Schema
      XMLComponentManager disableDTDComponent =
          new XMLComponentManager() {
  ...
  private void configureExternalDTDPipeline() {
    ...
    if (fSchemaValidator != null) {
      ...
      LSPXMLEntityManager entityManager =
          new LSPXMLEntityManager((AbstractLSPErrorReporter) fErrorReporter, null);
      AbstractLSPErrorReporter.initializeReporter(
          fSchemaValidator, getReporterForGrammar(), entityManager);
    }
  }
