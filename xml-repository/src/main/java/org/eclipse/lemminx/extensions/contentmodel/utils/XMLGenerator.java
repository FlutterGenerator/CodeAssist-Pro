...
/**
 * XML generator used to generate an XML fragment with formatting from a given element declaration
 * (XML Schema element declaration, DTD element, etc).
 */
public class XMLGenerator {
   ...
   * @param whitespacesIndent the whitespaces to use to indent XML children elements.
   * @param lineDelimiter the line delimiter to use when several XML elements must be generated.
   * @param canSupportSnippets true if snippets can be supported and false otherwise.
   */
  public XMLGenerator(
      ...
      int maxLevel) {
    this(
        sharedSettings, true, whitespacesIndent, lineDelimiter, canSupportSnippets, maxLevel, null);
  }
  ...
  public String generateMissingElements(
      ContentModelManager contentModelManager, DOMElement element, boolean generateOnlyRequired) {
    ...
    for (CMDocument cmDocument : contentModelManager.findCMDocument(element)) {
      ...
      if (matchesCMElement != null) {
        generatedXml.append(generate(matchesCMElement, null, true, true, 0, generateOnlyRequired));
      }
    }
  ...
  public String generate(
      CMElementDeclaration elementDeclaration, String prefix, boolean generateEndTag) {
    return generate(elementDeclaration, prefix, generateEndTag, false, 0, false);
  }

  /**
   * Returns the XML generated from the given element declaration given option to generate only
   * children and only required elements.
   *
   * @param elementDeclaration
   * @param prefix
   * @param generateEndTag
   * @param level the level at which the first element will be generated at
   * @param generateOnlyChildren true if only is children of the given element is to be generated
   * @param generateOnlyRequired true if only the required elements is to be generated
   * @return the XML generated from the given element declaration given option to generate only
   *     children.
   */
  public String generate(
    ...
    generate(
        ...
        0,
        xml,
        new HashMap<CMElementDeclaration, List<CMElementDeclaration>>(),
  ...
  private int generate(
    ...
    Collection<CMElementDeclaration> children =
        generateOnlyRequired
            ? elementDeclaration.getRequiredElements()
            : elementDeclaration.getElements();
    if (generateOnlyChildren) {
      for (CMElementDeclaration child : children) {
        List<CMElementDeclaration> parents =
            generatedElements.get(child) == null ? new ArrayList<>() : generatedElements.get(child);
    ...
    snippetIndex =
        generate(attributes, level, snippetIndex, xml, elementDeclaration.getLocalName());
    // Elements children
    if (children.size() > 0) {
      ...
      if ((level < maxLevel)) {
        for (CMElementDeclaration child : children) {
          level++;
          List<CMElementDeclaration> parents =
              generatedElements.get(child) == null
                  ? new ArrayList<>()
   ...
   *
   * <p>Can create an enumerated TextEdit if given a collection of values.
   */
   ...
   *
   * <p>If there is no content then null is returned.
   *
   * @param documentation
   * @param schemaURI
   * @return
   ...
   */
  public static MarkupContent createMarkupContent(
      CMElementDeclaration cmElement, ISharedSettingsRequest support) {
    String documentation =
        XMLGenerator.generateDocumentation(
            cmElement.getDocumentation(support),
            cmElement.getDocumentURI(),
  ...
  public static MarkupContent createMarkupContent(
    ...
    String documentation =
        XMLGenerator.generateDocumentation(
            cmAttribute.getAttributeNameDocumentation(request),
            ownerElement.getDocumentURI(),
   ...
   * @param ownerElement
   * @param support
   * @return a markup content for attribute value documentation and null otherwise.
   */
  public static MarkupContent createMarkupContent(
      CMAttributeDeclaration cmAttribute,
    ...
    String documentation =
        XMLGenerator.generateDocumentation(
            cmAttribute.getAttributeValueDocumentation(attributeValue, support),
            ownerElement.getDocumentURI(),
            support.canSupportMarkupKind(MarkupKind.MARKDOWN));
    if (documentation != null) {
      return MarkupContentFactory.createMarkupContent(documentation, MarkupKind.MARKDOWN, support);
   ...
   */
  public static MarkupContent createMarkupContent(
      CMElementDeclaration cmElement, String textContent, ISharedSettingsRequest support) {
    String documentation =
        XMLGenerator.generateDocumentation(
            cmElement.getTextDocumentation(textContent, support),
            cmElement.getDocumentURI(),
            support.canSupportMarkupKind(MarkupKind.MARKDOWN));
    if (documentation != null) {
      return MarkupContentFactory.createMarkupContent(documentation, MarkupKind.MARKDOWN, support);
