...
/**
 * Search node wraps a DOM node which matches a from/to path of a given {@link
 * XMLReferenceExpression}.
 *
 * <p>If the expression doesn't define multiple {@link XMLReferenceExpression#isMultiple()} the
 * search node contains the same ranges (start / end offsets) and the same value than the DOM node.
 * Ex : <code>
 * 	<link ref="#A" />
 * </code> In this sample the DOM node will generate one search node with the following range:
 * <code>
 * 	<link ref="|#A|" />
 * </code>
 *
 * <p>If the expression defines multiple {@link XMLReferenceExpression#isMultiple()} the DOM node
 * generates several search node with proper ranges and value. Ex : <code>
 * 	<link ref="#A #B" />
 * </code> In this sample the DOM node will generate two search nodes with the following ranges:
 * <code>
 * 	<link ref="|#A| |#B|" />
 * </code>
 *
...
public class SearchNode implements DOMRange {
  ...
  /**
   * Returns true if the given search node <code>searchNode</code> matches the value of this search
   * node and false otherwise.
   *
   * @param searchNode the search node.
   * @return true if the given search node <code>searchNode</code> matches the value of this search
   *     node and false otherwise.
   */
   ...
   *
   * <p>The node is valid when:
   *
   * <ul>
   *   <li>the search node is a 'to' node.
   *   <li>the 'from' search node have none prefix.
   *   <li>the 'from' search node have the expected prefix.
   * </ul>
   *
