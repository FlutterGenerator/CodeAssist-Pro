 ...
 *
 * <p>By default the manager is able to generate :
 *
 * <ul>
 *   <li>XML Schema from a given XML.
 *   <li>DTD from a given XML.
 * </ul>
 */
public class FileContentGeneratorManager {

  private final Map<Class<? extends FileContentGeneratorSettings>, IFileContentGenerator<?, ?>>
      generators;
   ...
   * @param generatorKey the key of the generator identified by the settings class.
   */
  public void registerGenerator(
      IFileContentGenerator<?, ?> generator,
      Class<? extends FileContentGeneratorSettings> generatorKey) {
    generators.put(generatorKey, generator);
  }

  /**
   * Generates a file content (ex : XSD, DTD) from the given document source <code>document</code>
   * (ex: XML) by using the given settings <code>generatorSettings</code>.
   *
   ...
   * @return the result of the generation of the file content (ex : XSD, DTD) from the given
   *     document source <code>document</code> (ex: XML) by using the given settings <code>
   *     generatorSettings</code>.
   */
  public <Source, Settings extends FileContentGeneratorSettings> String generate(
      Source document,
    ...
    IFileContentGenerator<Source, Settings> generator =
        (IFileContentGenerator<Source, Settings>) generators.get(generatorSettings.getClass());
    // process the generator
    return generator.generate(
        document, sharedSettings, generatorSettings, this.formatter, cancelChecker);
  }
}
