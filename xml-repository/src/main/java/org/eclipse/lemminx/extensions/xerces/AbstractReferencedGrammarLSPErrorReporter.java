/**
 * Copyright (c) 2021 Red Hat Inc. and others. All rights reserved. This program and the
...
public abstract class AbstractReferencedGrammarLSPErrorReporter extends AbstractLSPErrorReporter {
  ...
  public AbstractReferencedGrammarLSPErrorReporter(
      String source,
    ...
    this.referencedGrammarDiagnosticsInfoCache =
        referencedGrammarDiagnosticsInfoCache == null
            ? new HashMap<>()
  ...
  /**
   * Create a diagnostic root where XSD, DTD which have the error if needed and attach the error as
   * related information if LSP client support it.
   *
  ...
  protected void fillReferencedGrammarDiagnostic(
    ...
    ReferencedGrammarDiagnosticsInfo info =
        getReferencedGrammarDiagnosticsInfo(grammarURI, resolverExtensionManager);
    ...
    if (hasRelatedInformation && info.getGrammarDocument() != null) {
      ...
      if (schemaErrorCode != null) {
        range =
            XMLSchemaErrorCode.toLSPRange(location, schemaErrorCode, arguments, grammarDocument);
      ...
      if (range == null) {
        try {
          range =
              new Range(new Position(0, 0), grammarDocument.positionAt(grammarDocument.getEnd()));
   ...
   *
   * @param grammarURI the referenced grammar URI.
   * @param resolverExtensionManager the resolver used to load the DOM document of the referenced
   *     grammar.
   * @return
   */
  private ReferencedGrammarDiagnosticsInfo getReferencedGrammarDiagnosticsInfo(
      String grammarURI, URIResolverExtensionManager resolverExtensionManager) {
    ...
    if (info == null) {
      ...
      Diagnostic diagnostic =
          super.addDiagnostic(range, message, DiagnosticSeverity.Error, null, null);
    ...
    if (isReferencedGrammarError(key) && exception instanceof IOException) {
      ...
      if (cause instanceof CacheResourceException) {
        if (cause instanceof CacheResourceDownloadingException) {
          if (((CacheResourceDownloadingException) cause).getErrorCode()
