        ...
        for (CMDocument cmDocument : cmDocuments) {
          ...
          if (cmAttribute != null) {
            Range rangeValue =
                new Range(
                    new Position(
                        diagnosticRange.getStart().getLine(),
                        diagnosticRange.getStart().getCharacter() + 1),
                    new Position(
                        diagnosticRange.getEnd().getLine(),
            ...
            if (!similarValues.isEmpty()) {
              ...
              for (String similarValue : similarValues) {
                CodeAction similarCodeAction =
                    CodeActionFactory.replace(
                        "Did you mean '" + similarValue + "'?",
                        rangeValue,
                        similarValue,
              ...
              for (String otherValue : otherValues) {
                CodeAction otherCodeAction =
                    CodeActionFactory.replace(
                        "Replace with '" + otherValue + "'",
                        rangeValue,
                        otherValue,
                        document.getTextDocument(),
                        diagnostic);
