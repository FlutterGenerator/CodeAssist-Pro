...
public class CMXSDDocument implements CMDocument, XSElementDeclHelper {
  ...
  private final Map<CMXSDElementDeclaration, Map<XSTypeDefinition, CMXSDElementDeclaration>>
      refinedElementMappings;
  ...
  /**
   * Create files tracker to track all XML Schema (root and imported) from the XS model.
   *
   * @return a files tracker to track all XML Schema (root and imported) from the XS model.
   */
   ...
   */
  void collectElement(
      XSElementDeclaration elementDeclaration, Collection<CMElementDeclaration> elements) {
  ...
  public CMElementDeclaration findCMElement(DOMElement element, String namespace) {
    ...
    while (element != null && (namespace == null || namespace.equals(element.getNamespaceURI()))) {
      paths.add(0, element);
      element =
          element.getParentNode() instanceof DOMElement
              ? (DOMElement) element.getParentNode()
              : null;
    }
    ...
    for (int i = 0; i < paths.size(); i++) {
      ...
      if (i == 0) {
        declaration =
            (CMXSDElementDeclaration) findElementDeclaration(elt.getLocalName(), namespace);
      } else {
        declaration =
            (CMXSDElementDeclaration) declaration.findCMElement(elt.getLocalName(), namespace);
      }
      if (declaration == null) {
        break;
      }
      ...
      if (exactType != null) {
        ...
        Map<XSTypeDefinition, CMXSDElementDeclaration> refinedElementMappingsForDeclaration =
            refinedElementMappings.computeIfAbsent(baseDeclaration, _key -> new HashMap<>());
        ...
        if (possiblyQualifiedType.length == 1) {
          ...
          qualifiedType =
              new javax.xml.namespace.QName(
                  element.getNamespaceURI(possiblyQualifiedType[0]), possiblyQualifiedType[1]);
        }
        return (XSTypeDefinition)
            model.getComponents(XSConstants.TYPE_DEFINITION).get(qualifiedType);
      }
    }
    return null;
  }
  ...
  static XSObjectList getEnumerationAnnotations(
      XSSimpleTypeDefinition simpleTypeDefinition, String value) {
  ...
  public LocationLink findTypeLocation(DOMNode originNode) {
    ...
    CMXSDElementDeclaration elementDeclaration =
        (CMXSDElementDeclaration) findCMElement(originElement, originElement.getNamespaceURI());
    ...
    ElementImpl xercesElement =
        findLocalMappedXercesElement(elementDeclaration.getElementDeclaration(), xsLoader);
    ...
    String documentURI =
        xercesElement != null
            ? xercesElement.getOwnerDocument().getDocumentURI()
    ...
    if (URIUtils.isFileResource(documentURI)) {
      ...
      DOMDocument targetSchema =
          DOMUtils.loadDocument(
              documentURI, originNode.getOwnerDocument().getResolverExtensionManager());
      if (targetSchema == null) {
        return null;
      }
      if (originAttribute != null) {
        // find location of xs:attribute declaration
        CMXSDAttributeDeclaration attributeDeclaration =
            (CMXSDAttributeDeclaration) elementDeclaration.findCMAttribute(originAttribute);
        if (attributeDeclaration != null) {
          ...
          if (attributeDecl.getScope() == XSConstants.SCOPE_LOCAL) {
            return findLocalXSAttribute(
                originAttribute,
                targetSchema,
                attributeDecl.getEnclosingCTDefinition(),
                schemaGrammar);
          }
        }
      } else {
        ...
        if (globalElement) {
          ...
          if (xercesElement != null) {
            return findLocalXSElement(
                originElement, targetSchema, xercesElement.getCharacterOffset());
          }
          // 2) use the Xerces xs:complexType strategy
          XSComplexTypeDefinition complexTypeDefinition =
              elementDeclaration.getElementDeclaration().getEnclosingCTDefinition();
          if (complexTypeDefinition != null) {
            return findLocalXSElement(
                originElement, targetSchema, complexTypeDefinition, schemaGrammar);
          }
        }
      }
    }
  ...
  /**
   * Returns the owner schema grammar of the given XSD element declaration and null otherwise.
   *
   * @param elementDeclaration the XSD element declaration
   * @return the owner schema grammar of the given XSD element declaration and null otherwise.
   */
  SchemaGrammar getOwnerSchemaGrammar(XSElementDeclaration elementDeclaration) {
    ...
    XSNamespaceItem namespaceItem =
        enclosingType != null
            ? enclosingType.getNamespaceItem()
    ...
    if (elementDeclaration.getScope() == XSConstants.SCOPE_GLOBAL) {
      for (int i = 0; i < namespaces.getLength(); i++) {
        ...
        if (namespace instanceof SchemaGrammar
            && (elementDeclaration.equals(
                ((SchemaGrammar) namespace).getElementDeclaration(elementDeclaration.getName())))) {
  ...
  private static SchemaGrammar getSchemaGrammar(XSNamespaceItem namespaceItem) {
    return (namespaceItem != null && namespaceItem instanceof SchemaGrammar)
        ? (SchemaGrammar) namespaceItem
        : null;
  }

  /**
   * Returns the location of the global xs:element declared in the given XML Schema <code>
   * targetSchema</code> which matches the given XML element <code>originElement</code> and null
   * otherwise.
   *
   * @param originElement the XML element
   * @param targetSchema the XML Schema
   * @return the location of the global xs:element declared in the given XML Schema <code>
   *     targetSchema</code> which matches the given XML element <code>originElement</code> and null
   *     otherwise.
   */
  private static LocationLink findGlobalXSElement(
      DOMElement originElement, DOMDocument targetSchema) {
  ...
  /**
   * Returns the Xerces DOM xs:element which have created the given instance <code>
   * elementDeclaration</code> and null otherwise.
   *
   * @param elementDeclaration
   * @param xsLoader
   * @return the Xerces DOM xs:element which have created the given instance <code>
   *     elementDeclaration</code> and null otherwise
   */
  private static ElementImpl findLocalMappedXercesElement(
      XSElementDeclaration elementDeclaration, XSLoaderImpl xsLoader) {
    try {
      ...
      LOGGER.log(
          Level.SEVERE,
          "Error while retrieving mapped Xerces xs:element of '"
  ...
  /**
   * Returns the location link for the given origin element and target element at the given offset.
   *
   ...
   * @return the location link for the given origin element and target element at the given offset.
   */
  private static LocationLink findLocalXSElement(
      DOMElement originElement, DOMDocument targetSchema, int offset) {
  ...
  /**
   * Returns the index where the given element declaration is store in the particles array and null
   * otherwise.
   *
   * @param elementDeclaration the XS element declaration.
   * @param particles the XS particles declaration.
   * @return the index where the given element declaration is store in the particles array and null
   *     otherwise.
   */
  private static int getXSElementDeclIndex(
      XSElementDeclaration elementDeclaration, XSParticleDecl[] particles) {
  ...
  /**
   * Returns the location of the local xs:element declared in the given XML Schema <code>
   * targetSchema</code> which matches the given XML element <code>originElement</code> and null
   * otherwise.
   *
   * @param originElement the XML element
   * @param targetSchema the XML Schema
   * @param enclosingType the enclosing type of the XS element declaration which matches the XML
   *     element
  ...
  private static LocationLink findLocalXSElement(
      ...
      DOMDocument targetSchema,
      XSComplexTypeDefinition enclosingType,
  ...
  /**
   * Returns the offset where the local xs:complexType is declared and -1 otherwise.
   *
   * @param complexType the local complex type
   * @param grammar the grammar where local complex type is declared.
   * @return the offset where the local xs:complexType is declared and -1 otherwise.
   */
  private static int getComplexTypeOffset(
      XSComplexTypeDefinition complexType, SchemaGrammar grammar) {
    try {
      ...
      LOGGER.log(
          Level.SEVERE,
          "Error while retrieving offset of local xs:complexType'" + complexType.getName() + "'.",
          e);
    }
  ...
  private static XSComplexTypeDecl[] getXSComplexTypeDecls(SchemaGrammar grammar) {
    try {
      ...
      LOGGER.log(
          Level.SEVERE,
          "Error while retrieving list of xs:complexType of the grammar '"
              + grammar.getSchemaNamespace()
              + "'.",
          e);
      return null;
    }
  ...
  /**
   * Returns the location of the xs:element declared in the given XML Schema <code>targetSchema
   * </code> which matches the given XML element <code>originElement</code> and null otherwise.
   *
   * @param originElement the XML element
   * @param children the children where xs:element must be searched
   * @param inAnyLevel true if search must be done in any level and false otherwise.
   * @return
   */
  private static LocationLink findXSElement(
      DOMElement originElement, NodeList children, boolean inAnyLevel) {
    ...
    if (XSDUtils.isXSElement(elt)) {
      if (originElement.getLocalName().equals(elt.getAttribute("name"))) {
        ...
        LocationLink location =
            XMLPositionUtility.createLocationLink(originElement, targetAttr.getNodeAttrValue());
        return location;
      }
    }
    return null;
  ...
  private static LocationLink findLocalXSAttribute(
      DOMAttr originAttribute,
      DOMDocument targetSchema,
      XSComplexTypeDefinition enclosingType,
      SchemaGrammar schemaGrammar) {
    int complexTypeOffset = getComplexTypeOffset(enclosingType, schemaGrammar);
  ...
  private static LocationLink findXSAttribute(
      DOMAttr originAttribute, NodeList children, boolean inAnyLevel) {
    for (int i = 0; i < children.getLength(); i++) {
      Node n = children.item(i);
      if (n.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (XSDUtils.isXSAttribute(elt)) {
          if (originAttribute.getName().equals(elt.getAttribute("name"))) {
            DOMAttr targetAttr = (DOMAttr) elt.getAttributeNode("name");
            LocationLink location =
                XMLPositionUtility.createLocationLink(
                    originAttribute.getNodeAttrName(), targetAttr.getNodeAttrValue());
            return location;
          }
        }
        if (inAnyLevel && elt.hasChildNodes()) {
