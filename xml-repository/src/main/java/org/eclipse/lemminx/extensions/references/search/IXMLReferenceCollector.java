...
/**
 * API to collect from/to attribute, text nodes which matches {@link
 * XMLReferenceExpression#matchTo(org.w3c.dom.Node)}
 *
 * @author Angelo ZERR
 */
@FunctionalInterface
public interface IXMLReferenceCollector {
   ...
   * @param expression the reference expression which matches the from / to node.
   */
  void collect(
      SearchNode fromSearchNode, SearchNode toSearchNode, XMLReferenceExpression expression);
}
