 ...
 *
 * <ul>
 *   <li>Surround with Tags (Wrap)
 *   <li>Surround with Comments
 *   <li>Surround with CDATA
 * </ul>
 */
public class SurroundWithCommand extends AbstractDOMDocumentCommandHandler {
  ...
  public SurroundWithCommand(
      IXMLDocumentProvider documentProvider, ContentModelManager contentModelManager) {
  ...
  protected Object executeCommand(
    ...
    if (emptySelection && node.isElement()) {
      ...
      if (element.isInStartTag(offset) || element.isInEndTag(offset)) {
        ...
        endPos =
            document.positionAt(
                element.isEndTagClosed() ? element.getEndTagCloseOffset() + 1 : element.getEnd());
        adjusted = true;
      }
    }
  ...
  private List<String> getTags(DOMNode node, String prefix, int offset, boolean adjusted) {
    DOMElement parentElement =
        node.isElement() && !adjusted ? (DOMElement) node : node.getParentElement();
    Collection<CMDocument> cmDocuments =
        parentElement != null
            ? contentModelManager.findCMDocument(parentElement)
    ...
    for (CMDocument cmDocument : cmDocuments) {
      ...
      if (elementDeclaration != null) {
        return elementDeclaration.getPossibleElements(parentElement, offset).stream()
            .map(decl -> decl.getName(prefix))
            .sorted()
            .collect(Collectors.toList());
      }
