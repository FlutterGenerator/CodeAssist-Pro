...
public enum XMLSyntaxErrorCode implements IXMLErrorCode {
  ...
  public static Range toLSPRange(
      XMLLocator location, XMLSyntaxErrorCode code, Object[] arguments, DOMDocument document) {
    ...
    switch (code) {
      ...
      case AttributeNSNotUnique:
        {
          ...
          Range xmlns =
              XMLPositionUtility.selectAttributeNameFromGivenNameAt(
                  "xmlns:" + attrName, offset, document);
          if (xmlns != null) {
            return xmlns;
          }
          return XMLPositionUtility.selectAttributeNameFromGivenNameAt(attrName, offset, document);
      ...
      case AttributePrefixUnbound:
        {
          return XMLPositionUtility.selectAttributePrefixFromGivenNameAt(
              getString(arguments[1]), offset, document);
        }
      ...
      case SemicolonRequiredInReference:
        {
          EntityReferenceRange range =
              XMLPositionUtility.selectEntityReference(offset + 1, document, false);
      ...
      case OpenQuoteExpected:
        {
          ...
          if (node instanceof DTDDeclNode) {
            // ex : <!ATTLIST dadesadministratives idinstitut ID > <-- error on idinstitut which
            // must be quoted.
            String parameterName = getString(arguments[1] /* idinstitut*/);
            return XMLPositionUtility.selectParameterNameFromGivenName(
                parameterName, (DTDDeclNode) node);
          }
  ...
  /**
   * Remove the offset of the first character from the left offset which is not a whitespace.
   *
   * @param initialOffset the initial offset.
   * @param text the XML content.
   * @return the offset of the first character from the left offset which is not a whitespace.
   */
   ...
   */
  private static Range getRangeFromStartNodeToOffset(
      DOMNode fromNode, int toOffset, DOMDocument document) {
    ...
    if (fromNode.isElement()) {
      ...
      if (fromElement.hasTagName()) {
        ...
        endOffset =
            Math.max(
                endOffset, fromElement.getStartTagOpenOffset() + fromElement.getTagName().length());
      }
    }
    return XMLPositionUtility.createRange(startOffset, endOffset, document);
  }
  ...
  /**
   * Returns the first child element of the given element which matches the given tag name and null
   * otherwise.
   *
   * @param tagName the tag name.
   * @param element the DOM element.
   * @return the first child element of the given element which matches the given tag name and null
   *     otherwise.
   */
  ...
  public static void registerCodeActionParticipants(
    ...
    codeActions.put(
        RootElementTypeMustMatchDoctypedecl.getCode(),
        new RootElementTypeMustMatchDoctypedeclCodeAction());
    codeActions.put(
        SemicolonRequiredInReference.getCode(), new SemicolonRequiredInReferenceCodeAction());
