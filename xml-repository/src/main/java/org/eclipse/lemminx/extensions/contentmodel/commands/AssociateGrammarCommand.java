 ...
 *
 * <p>The command parameters {@link ExecuteCommandParams} must be filled with 3 parameters:
 *
 * <ul>
 *   <li>document URI (String) : the DOM document file URI to bind with a grammar.
 *   <li>grammar URI (String) : the XSD, DTD file URI to bind with the DOM document.
 *   <li>binding type (String) : which can takes values "standard", "xml-model" to know which
 *       binding type must be inserted in the DOM document.
 * </ul>
 *
 * @author Angelo ZERR
 */
public class AssociateGrammarCommand extends AbstractDOMDocumentCommandHandler {
  ...
  public static class UnknownBindingTypeException extends Exception {
    ...
    private static final String MESSAGE =
        "Unknown binding type ''{0}''. Allowed values are "
            + //
    ...
    if (GrammarBindingType.STANDARD.getName().equals(bindingType)) {
      if (isXSD) {
        ...
        if (StringUtils.isEmpty(targetNamespace)) {
          ...
          return NoGrammarConstraintsCodeAction.createXSINoNamespaceSchemaLocationEdit(
              grammarURI, document, sharedSettings);
        }
        // Insert inside <foo /> ->
        // xmlns="team_namespace"
        // xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
        // xsi:schemaLocation="team_namespace xsd/team.xsd"
        return NoGrammarConstraintsCodeAction.createXSISchemaLocationEdit(
            grammarURI, targetNamespace, document, sharedSettings);
      } else {
        // DTD file
        // Insert before <foo /> -> <!DOCTYPE foo SYSTEM "dtd/tag.dtd">
        return NoGrammarConstraintsCodeAction.createDocTypeEdit(
            grammarURI, document, sharedSettings);
      }
      ...
      return NoGrammarConstraintsCodeAction.createXmlModelEdit(
          grammarURI, targetNamespace, document, sharedSettings);
    }
  ...
  /**
   * SAX handler which extract the targetNamespace attribute from the xs:schema root tag element and
   * null otherwise.
   */
