...
/**
 * Custom XML entity manager which return an empty string when a DOCTYPE SYSTEM or ENTITY SYSTEM
 * doesn't exist to:
 *
 * <ul>
 *   <li>avoid breaking the XML syntax validation when XML contains a DOCTYPE/ENTITY SYSTEM which
 *       doesn't exist
 *   <li>report the error DOCTYPE/ENTITY SYSTEM which doesn't exist in the proper range of SYSTEM
 * </ul>
 *
 * @author Angelo ZERR
 */
public class LSPXMLEntityManager extends XMLEntityManager {
  ...
  private void reportError(String location, Exception e) {
    ...
    errorReporter.reportError(
        ...
        DTDErrorCode.DTDNotFound.getCode(),
        new Object[] {null, location},
  ...
  /**
   * The error code 'DTDNotFound' doesn't use the locator, but to avoid having some NPE, we create
   * an null locator.
   */
  private static final XMLLocator NULL_LOCATOR =
      new XMLLocator() {
