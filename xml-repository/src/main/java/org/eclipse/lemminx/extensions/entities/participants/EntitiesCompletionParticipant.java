...
public class EntitiesCompletionParticipant extends CompletionParticipantAdapter {
  ...
  public void onXMLContent(
      ...
      throws Exception {
    EntityReferenceRange entityRange =
        XMLPositionUtility.selectEntityReference(
            request.getOffset(), request.getXMLDocument(), false);
    if (entityRange == null) {
      return;
    }
   ...
   * @param markdown true if the documentation can be formatted as markdown and false otherwise.
   * @param response the completion response.
   */
  private static void collectLocalEntityProposals(
      DOMDocument document, Range entityRange, boolean markdown, ICompletionResponse response) {
    ...
    for (int i = 0; i < entities.getLength(); i++) {
      ...
      if (entity.getNodeName() != null) {
        ...
        MarkupContent documentation =
            EntitiesDocumentationUtils.getDocumentation(
                (DTDEntityDecl) entity, EntityOriginType.LOCAL, markdown);
   ...
   * @param response the completion response.
   */
  private static void collectExternalEntityProposals(
    ...
    for (CMDocument cmDocument : cmDocuments) {
      ...
      for (Entity entity : entities) {
        if (entity.getNodeName() != null) {
          ...
          MarkupContent documentation =
              EntitiesDocumentationUtils.getDocumentation(
                  (DTDEntityDecl) entity, EntityOriginType.EXTERNAL, markdown);
          fillCompletion(entity.getNodeName(), documentation, entityRange, response);
        }
      }
    }
   ...
   */
  private void collectPredefinedEntityProposals(
  ...
  private static void fillCompletion(
      String name, MarkupContent documentation, Range entityRange, ICompletionResponse response) {
