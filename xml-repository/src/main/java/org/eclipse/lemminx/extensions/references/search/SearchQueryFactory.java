...
public class SearchQueryFactory {
  ...
  /**
   * Returns the inversed direction of the given node according the given expression and query
   * direction.
   *
   * @param node the DOM node.
   * @param expression the reference expression.
   * @param queryDirection the query direction.
   * @return the inversed direction of the given node according the given expression and query
   *     direction.
   */
  public static Direction getInversedDirection(
      DOMNode node, XMLReferenceExpression expression, QueryDirection direction) {
    switch (direction) {
      case FROM_2_TO:
   ...
   * @param expression the reference expression.
   * @param queryDirection the query direction.
   * @return the direction of the given node according the given expression and query direction.
   */
  public static Direction getDirection(
      DOMNode node, XMLReferenceExpression expression, QueryDirection queryDirection) {
  ...
  public static SearchQuery createQuery(DOMNode node, int offset, XMLReferencesSettings settings) {
    ...
    SearchQuery query =
        internalCreateQuery(adjustedNode, offset, settings, QueryDirection.FROM_2_TO);
    if (query != null) {
      return query;
    }
  ...
  public static SearchQuery createFromQuery(
      DOMNode node, int offset, XMLReferencesSettings settings) {
    return createQuery(node, offset, settings, QueryDirection.FROM_2_TO);
  }

  public static SearchQuery createToQuery(
      DOMNode node, int offset, XMLReferencesSettings settings) {
    return createQuery(node, offset, settings, QueryDirection.TO_2_FROM);
  }

  public static SearchQuery createToQueryByRetrievingToBefore(
      DOMNode node, int offset, XMLReferencesSettings settings, CancelChecker cancelChecker) {
    ...
    if (query != null) {
      ...
      if (searchToNode != null) {
        ...
        List<XMLReferenceExpression> expressions =
            findExpressions(searchToNode.getNode(), settings, queryDirection);
  ...
  public static SearchQuery createQuery(
      DOMNode node, XMLReferencesSettings settings, QueryDirection direction) {
    return createQuery(node, -1, settings, direction);
  }

  public static SearchQuery createQuery(
      DOMNode node, int offset, XMLReferencesSettings settings, QueryDirection direction) {
    DOMNode adjustedNode = findAttrOrTextNode(node, offset);
    if (adjustedNode == null) {
      return null;
    }
  ...
  private static SearchQuery internalCreateQuery(
      DOMNode adjustedNode, int offset, XMLReferencesSettings settings, QueryDirection direction) {
    List<XMLReferenceExpression> expressions = findExpressions(adjustedNode, settings, direction);
    if (expressions == null) {
      return null;
    }
  ...
  /**
   * Returns XML references expressions which match the given DOM <code>document</code> and null
   * otherwise.
   *
   * @param document the DOM document.
   * @param xmlReferencesSettings the XML references settings which hosts all references
   *     expressions.
   * @return XML references expressions which match the given DOM <code>document</code> and null
   *     otherwise.
   */
  private static List<XMLReferenceExpression> findExpressions(
      DOMNode node, XMLReferencesSettings xmlReferencesSettings, QueryDirection queryDirection) {
    List<XMLReferences> allReferences =
        xmlReferencesSettings != null ? xmlReferencesSettings.getReferences() : null;
