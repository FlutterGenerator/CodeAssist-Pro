...
public interface IFilePathExpression {
   ...
   * @return true if the given DOM node matches the file path expression and false otherwise.
   */
  boolean match(Node node);

  /**
   * Returns the separator character (ex: ';') used to separate multiple files declaration (ex:
   * file1.xml;file2.xml) and null otherwise.
   *
   * @return the separator character (ex: ';') used to separate multiple files declaration (ex:
   *     file1.xml;file2.xml) and null otherwise.
   */
   ...
   * @return true if given file path is allowed for the file path completion and false otherwise.
   */
