...
public class XSDHighlightingParticipant implements IHighlightingParticipant {
  ...
  @Override
  public void findDocumentHighlights(
      DOMNode node,
      Position position,
      int offset,
    ...
    if (bindingType != BindingType.NONE) {
      ...
      highlights.add(
          new DocumentHighlight(
              XMLPositionUtility.createRange(
                  originAttr.getNodeAttrValue().getStart(),
                  originAttr.getNodeAttrValue().getEnd(),
                  document),
      ...
      XSDUtils.searchXSTargetAttributes(
          originAttr,
          bindingType,
          true,
          ...
          (targetNamespacePrefix, targetAttr) -> {
            highlights.add(
                new DocumentHighlight(
                    XMLPositionUtility.createRange(
                        targetAttr.getNodeAttrValue().getStart(),
                        targetAttr.getNodeAttrValue().getEnd(),
                        targetAttr.getOwnerDocument()),
      ...
      highlights.add(
          new DocumentHighlight(
              XMLPositionUtility.createRange(
                  ...
                  targetAttr.getOwnerDocument()),
              DocumentHighlightKind.Write));
      XSDUtils.searchXSOriginAttributes(
          targetAttr,
          (origin, target) ->
              highlights.add(
                  new DocumentHighlight(
                      XMLPositionUtility.createRange(
                          origin.getNodeAttrValue().getStart(),
                          origin.getNodeAttrValue().getEnd(),
                          origin.getOwnerDocument()),
                      DocumentHighlightKind.Read)),
          cancelChecker);
    }
  }
