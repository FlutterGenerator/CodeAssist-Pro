 ...
 *
 * <ul>
 *   <li>completion based on XML Schema, DTD...
 *   <li>hover based on XML Schema
 *   <li>diagnostics based on on XML Schema, DTD...
 * </ul>
 */
public class ContentModelPlugin implements IXMLExtension {
  ...
  private void updateSettings(ContentModelSettings settings, ISaveContext context) {
    ...
    if (settings.getFileAssociations() != null) {
      ...
      boolean fileAssociationsChanged =
          contentModelManager.setFileAssociations(settings.getFileAssociations());
    ...
    XMLDownloadExternalResourcesSettings downloadExternalResources =
        settings.getDownloadExternalResources();
    boolean downloadExternalResourcesEnabled =
        downloadExternalResources == null || downloadExternalResources.isEnabled();
    ...
    if (oldValidationSettings != null
        && !Objects.equals(oldValidationSettings, currentValidationSettings)) {
      context.collectDocumentToValidate(d -> true);
    }
    if (currentValidationSettings != null) {
      contentModelManager.setResolveExternalEntities(
          currentValidationSettings.isResolveExternalEntities());
    }
  }
  ...
  public void start(InitializeParams params, XMLExtensionsRegistry registry) {
    ...
    documentTelemetryParticipant =
        new DocumentTelemetryParticipant(registry.getTelemetryManager(), contentModelManager);
    ...
    if (commandService != null) {
      ...
      commandService.registerCommand(
          XMLValidationAllFilesCommand.COMMAND_ID,
          new XMLValidationAllFilesCommand(
              contentModelManager, documentProvider, validationService));
      commandService.registerCommand(
          AssociateGrammarCommand.COMMAND_ID, new AssociateGrammarCommand(documentProvider));
      commandService.registerCommand(
          CheckBoundGrammarCommand.COMMAND_ID, new CheckBoundGrammarCommand(documentProvider));
      commandService.registerCommand(
          CheckFilePatternCommand.COMMAND_ID, new CheckFilePatternCommand());
      commandService.registerCommand(
          SurroundWithCommand.COMMAND_ID,
