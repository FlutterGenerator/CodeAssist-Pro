...
public class RelaxNGUtils {
  ...
  /**
   * Returns the binding type of the origin attribute (ex : ref/@name) which bounds an another
   * target attribute (ex : define/@name).
   *
   * @param originAttr the origin attribute (ex : ref/@name)
   * @return the binding type of the origin attribute (ex : ref/@name) which bounds an another
   *     target attribute (ex : define/@name).
   */
  ...
  /**
   * Collect RNG target attributes declared in the RNG grammar according the given attribute and
   * binding type.
   *
   * @param originAttr the origin attribute.
   * @param matchAttr true if the attribute value must match the value of target attribute value and
   *     false otherwise.
   * @param searchInExternalSchema true if search must be done in included XML Schema (include) and
   *     false otherwise.
   * @param collector collector to collect RNG target attributes.
   */
  public static void searchRNGTargetAttributes(
      DOMAttr originAttr,
  ...
  private static void searchRNGTargetAttributes(
      DOMAttr originAttr,
      BindingType bindingType,
      boolean matchAttr,
      BiConsumer<String, DOMAttr> collector,
      ...
      for (String externalURI : externalURIS) {
        ...
        if (URIUtils.isFileResource(resourceURI)) {
          DOMDocument externalDocument =
              DOMUtils.loadDocument(resourceURI, document.getResolverExtensionManager());
          if (externalDocument != null) {
            searchRNGTargetAttributes(
                ...
                matchAttr,
                collector,
                externalDocument.getDocumentElement(),
                targetNamespacePrefix,
                originName,
                visitedURIs,
                searchInExternalSchema);
  ...
  private static boolean isBounded(
      Element originElement, BindingType originBinding, Element targetElement) {
   ...
   * @param collector the collector to collect reference between an origin and target attribute.
   */
  public static void searchRNGOriginAttributes(
      DOMNode targetNode, BiConsumer<DOMAttr, DOMAttr> collector, CancelChecker cancelChecker) {
  ...
  /**
   * Returns the target attributes list (ex : list of define/@name) from the given referenced node
   * (ex: DOM document).
   *
   * @param referencedNode the referenced node (ex: DOM document).
   * @return the target attributes list (ex : list of define/@name) from the given referenced node
   *     (ex: DOM document).
   */
  ...
  /**
   * Collect all target attributes (ex : define/@name) from the given parent DOM node.
   *
  ...
  private static void searchRNGOriginAttributes(
      NodeList nodes,
      List<DOMAttr> targetAttrs,
      String targetNamespacePrefix,
  ...
  /**
   * Returns true if the given element is an RNG target element (define element) and false
   * otherwise.
   *
   * @param element the DOM element.
   * @return true if the given element is an RNG target element (define element) and false
   *     otherwise.
   */
