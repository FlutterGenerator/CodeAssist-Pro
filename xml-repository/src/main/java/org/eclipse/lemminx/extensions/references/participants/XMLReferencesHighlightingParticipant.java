...
public class XMLReferencesHighlightingParticipant implements IHighlightingParticipant {
  ...
  public void findDocumentHighlights(
    ...
    SearchQuery query =
        SearchQueryFactory.createToQueryByRetrievingToBefore(
            node, offset, plugin.getReferencesSettings(), cancelChecker);
    if (query == null) {
    ...
    highlights.add(
        new DocumentHighlight(
            XMLPositionUtility.createRange(query.getSearchNode()), DocumentHighlightKind.Write));
    ...
    SearchEngine.getInstance()
        .search(
            ...
            (fromSearchNode, toSearchNode, expression) -> {
              highlights.add(
                  new DocumentHighlight(
                      XMLPositionUtility.createRange(fromSearchNode), DocumentHighlightKind.Read));
            },
            cancelChecker);
  }
}
