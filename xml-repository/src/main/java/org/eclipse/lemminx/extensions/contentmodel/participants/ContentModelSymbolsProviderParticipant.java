...
public class ContentModelSymbolsProviderParticipant implements ISymbolsProviderParticipant {
  ...
  @Override
  public void findSymbolInformations(
  ...
  @Override
  public void findDocumentSymbols(
      DOMDocument document,
    ...
    // those information
    DocumentSymbol rootSymbol =
        new DocumentSymbol(GRAMMARS_CATEGORY, SymbolKind.Module, DUMMY_RANGE, DUMMY_RANGE);
    ...
    for (ReferencedGrammarInfo info : referencedGrammarInfos) {
      ...
      DocumentSymbol grammarInfoSymbol =
          new DocumentSymbol(
              resolvedInfo.getResolvedURI(), SymbolKind.File, DUMMY_RANGE, DUMMY_RANGE);
      ...
      DocumentSymbol bindingInfoSymbol =
          new DocumentSymbol(bindingName.toString(), SymbolKind.Property, range, range);
      ...
      if (!info.isInCache()) {
        cacheInfoSymbol =
            new DocumentSymbol("Cache: false", SymbolKind.Property, DUMMY_RANGE, DUMMY_RANGE);
      } else {
        cacheInfoSymbol =
            new DocumentSymbol("Cache", SymbolKind.Property, DUMMY_RANGE, DUMMY_RANGE);
        cacheInfoSymbol.setChildren(symbols.createList());
        // Cache file symbol
        GrammarCacheInfo grammarCacheInfo = info.getGrammarCacheInfo();
        DocumentSymbol cacheFileSymbol =
            new DocumentSymbol(
                grammarCacheInfo.getCachedResolvedUri(), SymbolKind.File, DUMMY_RANGE, DUMMY_RANGE);
