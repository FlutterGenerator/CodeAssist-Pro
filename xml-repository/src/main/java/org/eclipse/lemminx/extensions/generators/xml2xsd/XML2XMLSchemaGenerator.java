...
public class XML2XMLSchemaGenerator
    extends AbstractXML2GrammarGenerator<XMLSchemaGeneratorSettings> {
  ...
  private void generateXSElement(
      XMLBuilder schema,
      String prefix,
    ...
    Cardinality cardinality =
        parentDecl != null ? parentDecl.getChildrenProperties().getCardinalities().get(name) : null;
    if (cardinality != null) {
    ...
    if (!hasChildren && !hasAttributes) {
      ...
      if (hasChildren) {
        ...
        if (allChildrenAreOptional(
            elementDecl.getChildrenProperties().getCardinalities().values())) {
