 ...
 *
 * <p>In this case, when XML file is not associated to a grammar/schema (DTD, XSD), this class
 * generates [Bind to grammar/schema...] CodeLens on the root of the DOM Document:
 *
 * <p>On client side, click on this Codelens should open a wizard:
 *
 * <ul>
 *   <li>page1 : display a combo to select the binding type ("standard", "xml-model").
 *   <li>page2: open a file dialog to select the grammar/schema (XSD/DTD) to bind.
 *   <li>the finish wizard should consume the "xml.associate.grammar.insert" command {@link
 *       AssociateGrammarCommand} to generate the proper syntax for binding with following
 *       parameters:
 *       <ul>
 *         <li>the document uri.
 *         <li>the selected grammar/schema file uri.
 *         <li>the binding type.
 *       </ul>
 *       In other words the "xml.associate.grammar.insert" returns the {@link TextDocumentEdit}
...
public class ContentModelCodeLensParticipant implements ICodeLensParticipant {
  ...
  @Override
  public void doCodeLens(
      ICodeLensRequest request, List<CodeLens> lenses, CancelChecker cancelChecker) {
  ...
  private void createReferencedGrammarLenses(ICodeLensRequest request, List<CodeLens> lenses) {
    ...
    Set<ReferencedGrammarInfo> referencedGrammarInfos =
        contentModelManager.getReferencedGrammarInfos(document);
  ...
  private CodeLens createReferencedGrammarLens(
      ReferencedGrammarInfo info, Range range, boolean canSupportOpenUri) {
  ...
  private static void createBindToGrammarSchemaLenses(
      ICodeLensRequest request, List<CodeLens> lenses) {
  ...
  private static boolean canSupportAssociation(ICodeLensRequest request) {
    ...
    return !DOMUtils.isXSD(document)
        && !DOMUtils.isDTD(document.getDocumentURI())
        && !DOMUtils.isRelaxNG(document);
  }
