...
public class SchemaProvider {
  ...
  public static Schema loadSchema(
      RelaxNGDescription description,
      XMLEntityResolver entityResolver,
      XMLErrorReporter errorReporter,
  ...
  public static Schema loadSchema(
    ...
    PropertyMap schemaProperties =
        createPropertyMap(entityResolver, errorReporter, schemaPatternBuilder, xmlReaderCreator);
    return schemaReader.createSchema(new SAXSource(input), schemaProperties);
  }

  private static InputSource createInputSource(
      RelaxNGDescription description, XMLEntityResolver entityResolver)
      throws MalformedURIException, IOException {
    XMLInputSource source = entityResolver.resolveEntity(description);
    return source.getByteStream() != null
        ? new InputSource(source.getByteStream())
        : new InputSource(source.getSystemId());
  }

  private static SchemaReader getSchemaReader(String systemId) {
    return DOMUtils.isRelaxNGUriCompactSyntax(systemId)
        ? MyCompactSchemaReader.getInstance()
          ...
          new EntityResolver() {
            ...
            public InputSource resolveEntity(String publicId, String systemId)
              ...
              XMLResourceIdentifier identifier =
                  new XMLResourceIdentifierImpl(publicId, systemId, systemId, systemId);
