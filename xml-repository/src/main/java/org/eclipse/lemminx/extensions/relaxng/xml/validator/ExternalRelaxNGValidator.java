...
public class ExternalRelaxNGValidator implements XMLComponent, XMLDocumentFilter {
  ...
  public static final String RELAXNG =
      "http://apache.org/xml/properties/relaxng/external-relaxng"; //$NON-NLS-1$ ;
    ...
    if (!processed) {
      try {
        if (externalRelaxNG != null && xmlReader != null) {
          ...
          try {
            location =
                XMLEntityManager.expandSystemId(externalRelaxNG, locator.getBaseSystemId(), false);
            Schema schema =
                SchemaProvider.getSchema(
                    externalRelaxNG,
                    locator.getBaseSystemId(),
                    entityResolver,
                    errorReporterForGrammar,
                    grammarPool);
            SchemaProvider.validate(schema, xmlReader, errorReporterForXML);
          } catch (IncorrectSchemaException e) {
            // ignore the error.
          } catch (Exception e) {
            errorReporterForXML.reportError(
                NULL_LOCATOR,
                RelaxNGConstants.RELAX_NG_DOMAIN,
                RelaxNGErrorCode.RelaxNGNotFound.getCode(),
      ...
      errorReporterForGrammar =
          (XMLErrorReporter)
              componentManager.getProperty(
                  XMLModelAwareParserConfiguration.ERROR_REPORTER_FOR_GRAMMAR);
    } catch (XMLConfigurationException e) {
