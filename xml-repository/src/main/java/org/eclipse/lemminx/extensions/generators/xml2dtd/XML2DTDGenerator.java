...
public class XML2DTDGenerator extends AbstractXML2GrammarGenerator<DTDGeneratorSettings> {
  ...
  protected void generate(
      Grammar grammar, DTDGeneratorSettings settings, XMLBuilder dtd, CancelChecker cancelChecker) {
      ...
      if (children.isEmpty()) {
        ...
        if (!hasCharacterContent) {
          ...
          if (sequenced) {
            ...
            dtd.addContent(" (");
            boolean first = true;
            for (Map.Entry<String, Cardinality> elementInfo :
                elementDecl.getChildrenProperties().getCardinalities().entrySet()) {
