...
public class XMLValidator {
  ...
  public static void doDiagnostics(
    ...
    Map<String, ReferencedGrammarDiagnosticsInfo> referencedGrammarDiagnosticsInfoCache =
        new HashMap<>();
    final LSPErrorReporterForXML reporterForXML =
        new LSPErrorReporterForXML(
            document,
            diagnostics,
            contentModelManager,
    ...
    // for XML and Grammar.
    final LSPErrorReporterForXML reporterForGrammar =
        new LSPErrorReporterForXML(
            document,
            diagnostics,
            contentModelManager,
    ...
    try {
      ...
      LSPXMLParserConfiguration configuration =
          new LSPXMLParserConfiguration(
              grammarPool,
      ...
      boolean hasSchemaGrammar =
          hasSchemaLocation || hasNoNamespaceSchemaLocation || hasExternalSchemaGrammar(document);
      boolean schemaValidationEnabled =
          (hasSchemaGrammar && isSchemaValidationEnabled(document, validationSettings)
              || (hasNoNamespaceSchemaLocation
                  && isNoNamespaceSchemaValidationEnabled(document, validationSettings)));
      parser.setFeature(
          "http://apache.org/xml/features/validation/schema",
          schemaValidationEnabled); //$NON-NLS-1$

      boolean hasRelaxNG = hasRelaxNGReference(document, parser);
      boolean hasGrammar =
          document.hasDTD() || hasSchemaGrammar || (!hasRelaxNG && document.hasExternalGrammar());
      if (hasSchemaGrammar && !schemaValidationEnabled) {
        hasGrammar = false;
      }
      parser.setFeature("http://xml.org/sax/features/validation", hasGrammar); // $NON-NLS-1$

      boolean namespacesValidationEnabled =
          isNamespacesValidationEnabled(document, validationSettings, hasRelaxNG);
      parser.setFeature("http://xml.org/sax/features/namespace-prefixes", false); // $NON-NLS-1$
      parser.setFeature(
          "http://xml.org/sax/features/namespaces", namespacesValidationEnabled); // $NON-NLS-1$
  ...
  private static boolean isNamespacesValidationEnabled(
      DOMDocument document, XMLValidationSettings validationSettings, boolean hasRelaxNG) {
    if (hasRelaxNG) {
      return true;
    }
  ...
  private static boolean isSchemaValidationEnabled(
      DOMDocument document, XMLValidationSettings validationSettings) {
    if (validationSettings == null) {
      return true;
    }
  ...
  /**
   * Returns true if the given DOM document declares a xsi:schemaLocation hint for the document root
   * element is valid and false otherwise.
   *
   * <p>The xsi:schemaLocation is valid if:
   *
   * <ul>
   *   <li>xsi:schemaLocation defines an URI for the namespace of the document element.
   *   <li>the URI can be opened
   * </ul>
   *
   * @param document the DOM document.
   * @return true if the given DOM document declares a xsi:schemaLocation hint for the document root
   *     element is valid and false otherwise.
   */
  ...
  private static boolean isNoNamespaceSchemaValidationEnabled(
      DOMDocument document, XMLValidationSettings validationSettings) {
    if (validationSettings == null) {
      return true;
    }
  ...
  /**
   * Returns true if the given DOM document declares a xsi:noNamespaceSchemaLocation which is valid
   * and false otherwise.
   *
   * <p>The xsi:noNamespaceSchemaLocation is valid if:
   *
   * <ul>
   *   <li>xsi:noNamespaceSchemaLocation defines an URI.
   *   <li>the URI can be opened
   * </ul>
   *
   * @param document the DOM document.
  ...
  private static boolean hasExternalSchemaGrammar(DOMDocument document) {
    ...
    return externalGrammarLocation.containsKey(
            IExternalGrammarLocationProvider.NO_NAMESPACE_SCHEMA_LOCATION)
  ...
  private static boolean isDisableOnlyDTDValidation(DOMDocument document) {
    ...
    return !docType.getChildren().stream()
        .anyMatch(node -> node.isDTDElementDecl() || node.isDTDAttListDecl());
  }
   ...
   */
  private static void warnNoGrammar(
      DOMDocument document,
      List<Diagnostic> diagnostics,
      XMLValidationSettings validationSettings) {
