...
/**
 * The SAX {@link ErrorHandler} gives just information of the offset where there is an error. To
 * improve highlight XML error, this class extends the Xerces XML reporter to catch location, key,
 * arguments which is helpful to adjust the LSP range.
 */
public abstract class AbstractLSPErrorReporter extends XMLErrorReporter {
  ...
  public AbstractLSPErrorReporter(
      String source,
    ...
    super.putMessageFormatter(
        XMLModelMessageFormatter.XML_MODEL_DOMAIN, new XMLModelMessageFormatter());
  }
  ...
  public String reportError(
    ...
    DiagnosticSeverity diagnosticSeverity =
        getSeverity(domain, key, arguments, severity, exception);
    Range adjustedRange =
        internalToLSPRange(
            location,
            key,
    ...
    if (hasRelatedInfo) {
      try {
        relatedInformations =
            AggregateRelatedInfoFinder.getInstance()
                .findRelatedInformation(
                    xmlDocument.offsetAt(adjustedRange.getStart()), key, xmlDocument);
      } catch (BadLocationException e) {
        LOGGER.severe("Passed bad Range: " + e);
      }
    }
    String code = getCode(domain, key, arguments, exception);
    if (addDiagnostic(adjustedRange, message, diagnosticSeverity, code, relatedInformations)
        == null) {
      return null;
    }
  ...
  protected DiagnosticSeverity getSeverity(
  ...
  public Diagnostic addDiagnostic(
      Range adjustedRange,
      String message,
   ...
   * @param fatalError true if Xerces report the error as fatal and false otherwise.
   * @param exception
   * @param document the DOM document.
   * @return the LSP range from the SAX error.
   */
  private Range internalToLSPRange(
      XMLLocator location,
      String key,
      Object[] arguments,
      String message,
    ...
    if (location == null) {
      if (exception instanceof XMLParseException) {
        ...
        location =
            new SimpleLocator(
                ex.getLiteralSystemId(),
                ex.getExpandedSystemId(),
                ex.getLineNumber(),
    ...
    Range range =
        toLSPRange(location, key, arguments, message, diagnosticSeverity, fatalError, document);
    if (range != null) {
      return range;
    }
  ...
  /**
   * Returns the range of the given error information, or {{@link #NO_RANGE} if diagnostic must not
   ...
   * @return
   */
  protected abstract Range toLSPRange(
      XMLLocator location,
      String key,
      Object[] arguments,
      String message,
  ...
  private static Position toLSPPosition(int offset, XMLLocator location, TextDocument document) {
    ...
    try {
      ...
      return location != null
          ? new Position(location.getLineNumber() - 1, location.getColumnNumber() - 1)
          : null;
    }
  }
  ...
  public static boolean initializeReporter(
      XMLSchemaValidator schemaValidator,
      XMLErrorReporter reporter,
      XMLEntityManager entityManager) {
    try {
      XMLSchemaLoader schemaLoader =
          ReflectionUtils.getFieldValue(schemaValidator, "fSchemaLoader");
  ...
  public static boolean initializeReporter(
      XMLSchemaLoader schemaLoader, XMLErrorReporter reporter, XMLEntityManager entityManager) {
    try {
      ...
      if (reporter != null) {
        schemaParser.setProperty(
            "http://apache.org/xml/properties/internal/error-reporter", reporter);
      }
      return true;
    } catch (Exception e) {
      LOGGER.log(Level.SEVERE, "Error while initializing XML error reporter", e);
