...
/**
 * Formatter participant implementation to format xsi:schemaLocation attribute value. The format of
 * the xsi:schemaLocation attribute value depends on the {@link XSISchemaLocationSplit} setting:
 *
 * <ul>
 *   <li>{@link XSISchemaLocationSplit#none} : don't format the xsi:schemaLocation attribute value.
 *   <li>{@link XSISchemaLocationSplit#onElement} : generate a line feed for each namespace
 *       declaration:
 *       <pre>
 * &lt;beans
 ...
 * </pre>
 *   <li>{@link XSISchemaLocationSplit#onPair} : generate a line feed for each location declaration:
 *       <pre>
 * &lt;beans
 * xmlns="http://www.springframework.org/schema/beans"
 * xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
...
public class XSIFormatterParticipant implements IFormatterParticipant {
  ...
  @Override
  public boolean formatAttributeValue(
      String name, String valueWithoutQuote, Character quote, DOMAttr attr, XMLBuilder xml) {
  ...
  public boolean formatAttributeValue(
    ...
    if (split == XSISchemaLocationSplit.none
        || !XSISchemaModel.isXSISchemaLocationAttr(attr.getName(), attr)) {
      if (formatterDocument.isMaxLineWidthSupported() && attr.getValue() != null) {
        parentConstraints.setAvailableLineWidth(
            parentConstraints.getAvailableLineWidth() - attr.getValue().length());
      }
      return false;
    }
    ...
    // and actual value
    formatterDocument.removeLeftSpaces(
        attrValueStart + 1, // <... xsi:schemaLocation="| value"
        // <... xsi:schemaLocation=" |value"
        attrValueStart + firstContentOffset,
        edits);
    ...
    if (formattingOptions.getSplitAttributes() != SplitAttributes.preserve) {
      if (formattingOptions.getSplitAttributes() == SplitAttributes.splitNewLine) {
        indentSpaceOffset =
            (attrValueStart + 1)
                - attr.getNodeAttrName().getStart()
                + formattingOptions.getSplitAttributesIndentSize() * tabSize;
      } else {
        indentSpaceOffset =
            (attrValueStart + 1)
                - attr.getNodeAttrName().getStart()
                + attr.getOwnerElement().getTagName().length()
                + 2;
      }
    } else if (formattingOptions.isPreserveAttributeLineBreaks()) {
      indentSpaceOffset =
          attrValueStart
              - formatterDocument.getOffsetWithPreserveLineBreaks(
                  startOfLineOffset, attrValueStart, tabSize, formattingOptions.isInsertSpaces());
    } else {
      indentSpaceOffset =
          formatterDocument.getNormalizedLength(startOfLineOffset, attrValueStart + 1)
              - startOfLineOffset;
    }
    ...
    for (int i = firstContentOffset; i < attrValue.length(); i++) {
      int from =
          formatterDocument.adjustOffsetWithLeftWhitespaces(attrValueStart, attrValueStart + i + 1);
      if (Character.isWhitespace(attrValue.charAt(i))
          && !Character.isWhitespace(attrValue.charAt(i + 1))
        ...
        if (availableLineWidth < 0
            && formatterDocument.isMaxLineWidthSupported()
        ...
        if (locationNum % lineFeed == 0) {
          formatterDocument.replaceLeftSpacesWithIndentationWithOffsetSpaces(
              indentSpaceOffset, attrValueStart, attrValueStart + i + 1, true, edits);
          availableLineWidth = formatterDocument.getMaxLineWidth() - attrValuelength;
        } else {
          formatterDocument.replaceLeftSpacesWithOneSpace(
              indentSpaceOffset, attrValueStart + i + 1, edits);
  ...
  /**
   * Returns the offset from opening quote to first non-whitespace character of an attribute value
   *
   * @param originalValue
   * @return offset from opening quote to first non-whitespace character of an attribute value
   */
  private static int getFirstContentOffset(String originalValue) {
    ...
    for (int i = 1; i < originalValue.length(); i++) {
      if (!Character.isWhitespace(originalValue.charAt(i))
          && !StringUtils.isQuote(originalValue.charAt(i))) {
        return i;
      }
    }
    return -1;
