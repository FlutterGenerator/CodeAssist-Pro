...
public class XMLReferencesDefinitionParticipant extends AbstractDefinitionParticipant {
  ...
  protected void doFindDefinition(
    ...
    SearchQuery query =
        SearchQueryFactory.createFromQuery(
            request.getNode(), request.getOffset(), plugin.getReferencesSettings());
    if (query == null) {
            ...
            (fromSearchNode, toSearchNode, expression) -> {
              LocationLink location =
                  XMLPositionUtility.createLocationLink(
                      XMLPositionUtility.createRange(fromSearchNode), toSearchNode);
