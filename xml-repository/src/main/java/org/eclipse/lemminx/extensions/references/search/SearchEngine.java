...
public class SearchEngine {
  ...
  public final void search(
      SearchQuery query, IXMLReferenceCollector collector, CancelChecker cancelChecker) {
  ...
  public Collection<ReferenceLink> searchLinks(
      DOMDocument document, XMLReferencesSettings settings, CancelChecker cancelChecker) {
    SearchQuery query = SearchQueryFactory.createQuery(document, settings, QueryDirection.BOTH);
    if (query != null) {
   ...
   * @param query the search query.
   * @param collector the collector used to collect attribute, text nodes.
   * @param visitedURIs visited URis used to avoid document loading recursion when document contains
   *     some xi:include.
   * @param cancelChecker the cancel checker.
   */
  private void searchInDocument(
  ...
  /**
   * Returns true if the search can be performed for the given document URI and false otherwise.
   *
   * @param documentURI the document URI.
   * @param visitedURIs the visited URIs.
   * @return true if the search can be performed for the given document URI and false otherwise.
   */
   ...
   * @param cancelChecker the cancel checker.
   */
  private void searchInNode(
      DOMNode node,
      SearchQuery query,
      IXMLReferenceCollector collector,
      Set<String> externalURIs,
      CancelChecker cancelChecker) {
   ...
   * @param cancelChecker the cancel checker.
   */
  private void searchInAttributes(
      DOMElement element,
      SearchQuery query,
      IXMLReferenceCollector collector,
      CancelChecker cancelChecker) {
  ...
  private List<SearchNode> findSearchNodes(
      DOMNode node, XMLReferenceExpression expression, Direction nodeDirection) {
    if (nodeDirection == Direction.FROM) {
      return SearchNodeFactory.findSearchNodes(
          node, expression.getPrefix(), expression.isMultiple(), nodeDirection);
    }
    return SearchNodeFactory.findSearchNodes(node, null, false, nodeDirection);
  }

  /**
   * Returns true if the given element is an include element (ex : xi:include) and false otherwise.
   *
   * @param element the DOM element.
   * @return true if the given element is an include element (ex : xi:include) and false otherwise.
   */
