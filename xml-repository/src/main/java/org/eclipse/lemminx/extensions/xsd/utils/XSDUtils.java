...
public class XSDUtils {
   ...
   * @return the binding type of the origin attribute which bounds an another target attribute.
   */
  ...
  /**
   * Collect XSD target attributes declared in the XML Schema according the given attribute and
   * binding type.
   *
   * @param originAttr the origin attribute.
   * @param matchAttr true if the attribute value must match the value of target attribute value and
   *     false otherwise.
   * @param searchInExternalSchema true if search must be done in included XML Schema (xs:include)
   *     and false otherwise.
   * @param collector collector to collect XSD target attributes.
   */
  public static void searchXSTargetAttributes(
      DOMAttr originAttr,
  ...
  private static void searchXSTargetAttributes(
      DOMAttr originAttr,
      BindingType bindingType,
      boolean matchAttr,
      BiConsumer<String, DOMAttr> collector,
   ...
   */
  public static void searchXSOriginAttributes(
  ...
  private static void searchXSOriginAttributes(
      NodeList nodes,
  ...
  public static boolean isXSTargetElement(Element element) {
    return isXSComplexType(element)
        || isXSSimpleType(element)
        || isXSElement(element)
        || isXSGroup(element);
  }
  ...
  private static void updateTracker(
      ...
      Set<SchemaGrammar> trackedGrammars,
      Set<String> trackedURIs,
   ...
   * <code>document</code> by the given <code>grammarURI</code> and null otherwise.
   *
   * @param document the XSD document
   * @param grammarURI the grammar URI to retreive.
   * @return the xsd:import/@schemaLocation or xsd:include/@schemaLocation declared in the given
   *     <code>document</code> by the given <code>grammarURI</code> and null otherwise
   */
        ...
        if (child.isElement()) {
          ...
          if (XSDUtils.isXSInclude(xsdElement) || XSDUtils.isXSImport(xsdElement)) {
            ...
            if (schemaLocationAttr != null) {
              ...
              if (grammarURI.equals(attrValue)
                  || ((grammarURI.endsWith(attrValue)
                      && grammarURI.equals(
                          getResolvedLocation(document.getDocumentURI(), attrValue))))) {
