...
public class SearchQuery {
  ...
  public SearchQuery(
      SearchNode searchNode, List<XMLReferenceExpression> expressions, QueryDirection direction) {
  ...
  private SearchQuery(
    ...
    if (searchNode != null) {
      ...
      this.searchNode =
          SearchNodeFactory.getSearchNodeAt(node, offset, prefix, hasMultiple, searchNodeDirection);
    }
  }
  ...
  private static boolean isSearchInAttribute(
      XMLReferenceExpression expression, QueryDirection direction) {
    switch (direction) {
      case FROM_2_TO:
  ...
  private static boolean isSearchInText(
      XMLReferenceExpression expression, QueryDirection direction) {
    switch (direction) {
      case FROM_2_TO:
