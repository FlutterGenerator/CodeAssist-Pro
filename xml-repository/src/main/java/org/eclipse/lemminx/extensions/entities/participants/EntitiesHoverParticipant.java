...
public class EntitiesHoverParticipant extends HoverParticipantAdapter {
  ...
  /**
   * Returns the markup content of the given entity name in the predefined, local and external
   ...
   * @return the markup content of the given entity name in the predefined, local and external
   *     entities and null otherwise.
   */
  private static MarkupContent searchInEntities(
      String entityName, Range entityRange, DOMDocument document, IHoverRequest request) {
    MarkupContent entityContents =
        searchInPredefinedEntities(entityName, entityRange, document, request);
    if (entityContents != null) {
  ...
  /**
   * Returns the markup content of the given entity name in the predefined entities and null
   * otherwise.
   *
   * @param entityName the entity name to search.
   * @param entityRange the hovered range.
   * @param document the DOM document
   * @param request the hover request.
   * @return the markup content of the given entity name in the predefined entities and null
   *     otherwise.
   */
  private static MarkupContent searchInPredefinedEntities(
      String entityName, Range entityRange, DOMDocument document, IHoverRequest request) {
    ...
    for (PredefinedEntity entity : entities) {
      if (entityName.equals(entity.getName())) {
        ...
        return EntitiesDocumentationUtils.getDocumentation(
            entity.getName(), entity.getValue(), EntityOriginType.PREDEFINED, markdown);
      }
    }
    return null;
  }
  ...
  /**
   * Returns the markup content of the given entity name in the local entities and null otherwise.
   *
   * @param entityName the entity name to search.
   * @param entityRange the hovered range.
   * @param document the DOM document
   * @param request the hover request.
   * @return the markup content of the given entity name in the local entities and null otherwise.
   */
  private static MarkupContent searchInLocalEntities(
      String entityName, Range entityRange, DOMDocument document, IHoverRequest request) {
    ...
    for (int i = 0; i < entities.getLength(); i++) {
      ...
      if (entityName.equals(entity.getName())) {
        boolean markdown = request.canSupportMarkupKind(MarkupKind.MARKDOWN);
        return EntitiesDocumentationUtils.getDocumentation(
            entity, EntityOriginType.LOCAL, markdown);
      }
    }
    return null;
  }
  ...
  /**
   * Returns the markup content of the given entity name in the external entities and null
   * otherwise.
   *
   * @param entityName the entity name to search.
   * @param entityRange the hovered range.
   * @param document the DOM document
   * @param request the hover request.
   * @return the markup content of the given entity name in the external entities and null
   *     otherwise.
   */
  private static MarkupContent searchInExternalEntities(
      String entityName, Range entityRange, DOMDocument document, IHoverRequest request) {
    ...
    for (CMDocument cmDocument : cmDocuments) {
      ...
      for (Entity ent : entities) {
        ...
        if (entityName.equals(entity.getName())) {
          boolean markdown = request.canSupportMarkupKind(MarkupKind.MARKDOWN);
          return EntitiesDocumentationUtils.getDocumentation(
              entity, EntityOriginType.EXTERNAL, markdown);
        }
      }
    }
    return null;
