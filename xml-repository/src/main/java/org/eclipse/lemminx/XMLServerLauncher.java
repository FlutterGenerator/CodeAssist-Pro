...
public class XMLServerLauncher {
  ...
  /**
   * Calls {@link #launch(InputStream, OutputStream)}, using the standard input and output streams.
   */
  ...
  /**
   * Launches {@link XMLLanguageServer} and makes it accessible through the JSON RPC protocol
   * defined by the LSP.
   *
   * @param launcherFuture The future returned by {@link
   *     org.eclipse.lsp4j.jsonrpc.Launcher#startListening()}. (I'm not 100% sure how it meant to be
   *     used though, as it's undocumented...)
   */
  public static Future<?> launch(InputStream in, OutputStream out) {
    ...
    Launcher<LanguageClient> launcher =
        createServerLauncher(server, in, out, Executors.newCachedThreadPool(), wrapper);
    server.setClient(launcher.getRemoteProxy());
    return launcher.startListening();
  }

  /**
   * Create a new Launcher for a language server and an input and output stream. Threads are started
   * with the given executor service. The wrapper function is applied to the incoming and outgoing
   * message streams so additional message handling such as validation and tracing can be included.
   *
   ...
   */
  private static Launcher<LanguageClient> createServerLauncher(
