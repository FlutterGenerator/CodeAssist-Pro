...
public class TreeViewAdapter<D> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
  ...
  TreeViewAdapter(
      Context context, TreeNode<D> root, @NonNull BaseNodeViewFactory<D> baseNodeViewFactory) {
  ...
  public int getItemViewType(int position) {
    // return expandedNodeList.get(position).getLevel(); // this old code row used to always return
    // the level
  ...
  private void setupCheckableItem(
      View nodeView, final TreeNode<D> treeNode, final CheckableNodeViewBinder<D> viewBinder) {
    final View view = nodeView.findViewById(viewBinder.getCheckableViewId());

    if (view instanceof Checkable checkableView) {
  ...
  private void selectChildren(TreeNode<D> treeNode, boolean checked) {
    ...
    if (index != -1 && !impactedChildren.isEmpty()) {
  ...
  /**
   * Refresh all,this operation is only used for refreshing list when a large of nodes have changed
   * value or structure because it takes much calculation.
   */
