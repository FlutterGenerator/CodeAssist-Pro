...
// *
// * <p>When a user executes a wrapper script the first time, the script downloads and installs the
// * appropriate Gradle distribution and runs the build against this downloaded distribution. Any
// * installed Gradle distribution is ignored when using the wrapper scripts.
// *
// * <p>The scripts generated by this task are intended to be committed to your version control
// * system. This task also generates a small {@code gradle-wrapper.jar} bootstrap JAR file and
// * properties file which should also be committed to your VCS. The scripts delegates to this JAR.
// */
//public class IDEWrapperGenerator {
//
//  public static final String DEFAULT_DISTRIBUTION_PARENT_NAME = WrapperDefaults.DISTRIBUTION_PATH;
//
//  private File scriptFile = new File(WrapperDefaults.SCRIPT_PATH);
//  private File jarFile = new File(WrapperDefaults.JAR_FILE_PATH);
...
//  private final int networkTimeout = WrapperDefaults.NETWORK_TIMEOUT;
//  private boolean distributionUrlConfigured = false;
//
//  void generate(Project project) {
//    final var isOffline = project.getGradle().getStartParameter().isOffline();
//    File jarFileDestination = getJarFile();
//    File unixScript = getScriptFile();
//    File propertiesFile = getPropertiesFile();
//    Properties existingProperties =
//        propertiesFile.exists() ? GUtil.loadProperties(propertiesFile) : null;
//
//    checkProperties(existingProperties);
//    validateDistributionUrl(propertiesFile.getParentFile(), isOffline);
//
//    WrapperGenerator.generate(archiveBase, archivePath, distributionBase, distributionPath,
//        getDistributionSha256Sum(existingProperties), propertiesFile, jarFileDestination,
//        FilesKt.relativeTo(jarFileDestination, unixScript.getParentFile()).getPath(), unixScript, getBatchScript(), getDistributionUrl(),
//        getValidateDistributionUrl(), networkTimeout);
//  }
//
//  private void checkProperties(Properties existingProperties) {
//    String checksumProperty = existingProperties != null ? existingProperties.getProperty(
//        WrapperExecutor.DISTRIBUTION_SHA_256_SUM, null) : null;
//
//    if (!isCurrentVersion() && distributionSha256Sum == null && checksumProperty != null) {
//      throw new GradleException(
//          "gradle-wrapper.properties contains distributionSha256Sum property, but the wrapper configuration does not have one. Specify one in the wrapper task configuration or with the --gradle-distribution-sha256-sum task option");
//    }
//  }
//
...
//
//  /**
//   * Returns the path where the gradle distributions needed by the wrapper are unzipped. The path is
//   * relative to the distribution base directory
//   *
...
//
//  /**
//   * Sets the path where the gradle distributions needed by the wrapper are unzipped. The path is
//   * relative to the distribution base directory
//   *
//   * @see #setDistributionPath(String)
//   */
...
//   *
//   * @throws GradleException if the label that can be provided via {@link #setGradleVersion(String)}
//   *                         can not be resolved at the moment. For example, there is not a
//   *                         `release-candidate` available at all times.
...
//
//  /**
//   * The version of the gradle distribution required by the wrapper. This is usually the same
//   * version of Gradle you use for building your project. The following labels are allowed to
//   * specify a version: {@code latest}, {@code release-candidate}, {@code nightly}, and
//   * {@code release-nightly}
//   *
//   * <p>The resulting distribution url is validated before it is written to the
//   * gradle-wrapper.properties file.
//   */
//  @Option(option = "gradle-version", description =
...
//
//  /**
//   * The type of the Gradle distribution to be used by the wrapper. By default, this is
//   * {@link DistributionType#BIN}, which is the binary-only Gradle distribution without
//   * documentation.
//   *
...
//   *
//   * <p>The wrapper downloads a certain distribution only once and caches it. If your distribution
//   * base is the project, you might submit the distribution to your version control system. That way
//   * no download is necessary at all. This might be in particular interesting, if you provide a
//   * custom gradle snapshot to the wrapper, because you don't need to provide a download server
//   * then.
//   */
//
...
//    }
//
//    return WrapperGenerator.getDistributionUrl(gradleVersionResolver.getGradleVersion(),
//        distributionType);
//  }
//
//  /**
...
//   * <p>If {@link #getGradleVersion()} is not set, will return null.
//   *
//   * <p>The wrapper downloads a certain distribution and caches it. If your distribution base is
//   * the project, you might submit the distribution to your version control system. That way no
//   * download is necessary at all. This might be in particular interesting, if you provide a custom
//   * gradle snapshot to the wrapper, because you don't need to provide a download server then.
//   *
//   * <p>The distribution url is validated before it is written to the gradle-wrapper.properties
//   * file.
//   */
...
//   *
//   * <p>The wrapper allows for verification of the downloaded Gradle distribution via SHA-256 hash
//   * sum comparison. This increases security against targeted attacks by preventing a
//   * man-in-the-middle attacker from tampering with the downloaded Gradle distribution.
//   *
//   * @since 4.5
//   */
//  @Nullable
...
//
//  /**
//   * The distribution base specifies whether the unpacked wrapper distribution should be stored in
//   * the project or in the gradle user home dir.
//   */
//
...
//   * The path is relative to the archive base directory.
//   */
//
...
//   * The path is relative to the parent dir specified with {@link #getArchiveBase()}.
//   */
...
//
//  /**
//   * The archive base specifies whether the unpacked wrapper distribution should be stored in the
...
//
//  /**
//   * The network timeout specifies how many ms to wait for when the wrapper is performing network
//   * operations, such as downloading the wrapper jar.
//   *
//   * @since 7.6
//   */
//
