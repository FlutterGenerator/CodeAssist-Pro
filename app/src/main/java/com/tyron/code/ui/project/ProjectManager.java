    ...
    if (res.exists()) {
      ...
      if (indexFiles.containsKey(RES)) {
        ...
        try {
          ...
          task.run();
        } catch (IOException | CompilationFailedException ignored) {
        }
      }

      if (res.exists()) {
        if (indexFiles.containsKey(XML)) {
...
//                    mCurrentProject.getEventManager().dispatchEvent(new XmlReparsedEvent(null));
  ...
  private synchronized void indexModule(Module module) throws IOException {
      //  module.open();
      //  module.index();
         if(!(module instanceof JavaModule javaModule))return;
  ...
  public static File createFile(File directory, String name, CodeTemplate template)
    ...
    FileUtils.writeStringToFile(classFile, code, UTF_8);
    return classFile;
  }
      ...
      if (content != null) {
        if (content.contains("namespace") && !content.contains("applicationId")) {
          ...
          return packageName;
           ...
           return module.getApplicationId();
        } else {
          throw new IOException(
              "Unable to find namespace or applicationId in "
                  + module.getModuleName()
