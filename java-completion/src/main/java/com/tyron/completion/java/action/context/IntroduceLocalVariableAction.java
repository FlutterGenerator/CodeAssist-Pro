...
public class IntroduceLocalVariableAction extends AnAction {
  ...
  public void actionPerformed(@NonNull AnActionEvent e) {
    ...
    TreePath currentPath =
        new FindCurrentPath(javacTask)
            .scan(unit, editor.getCaret().getStart(), editor.getCaret().getEnd());
  ...
  private JavaRewrite2 performInternal(JavacTaskImpl task, TreePath path, File file) {
    ...
    if (element instanceof ExecutableElement) {
      TypeMirror returnType = ActionUtil.getReturnType(task, path, (ExecutableElement) element);
